...
public final class SVGExporter {
    ...
    private static void exportInternal(final WorkflowEditor editor, final File file)
        ...
        if (viewer == null) {
            NodeLogger.getLogger(SVGExporter.class).debug(Messages.SVGExporter_0);
            return;
        }
        ...
        Writer fileOut =  new BufferedWriter(new OutputStreamWriter(new FileOutputStream(file), "UTF-8")); //$NON-NLS-1$
        fileOut.write("<?xml version=\"1.0\" encoding=\"UTF-8\"?>"); //$NON-NLS-1$
        ...
        if (dpix > 72) {
            try {
                XPathExpression xpath = XPathFactory.newInstance().newXPath().compile("//@font-size"); //$NON-NLS-1$
