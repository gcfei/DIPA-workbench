...
public class SVGExportWizard extends ExportWizard implements IExportWizard {
    ...
    private final ExportToFilePage m_page = new ExportToFilePage(Messages.SVGExportWizard_0,
        Messages.SVGExportWizard_1);
    ...
    public SVGExportWizard() {
        super();
        setWindowTitle(Messages.SVGExportWizard_2);
        m_page.addFileExtensionFilter("*.svg", Messages.SVGExportWizard_4); //$NON-NLS-1$
    }
    ...
    public boolean performFinish() {
        if (!(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().getActiveEditor() instanceof WorkflowEditor)) {
            ...
            mb.setText(Messages.SVGExportWizard_5);
            mb.setMessage(Messages.SVGExportWizard_6);
            mb.open();
            return false;
        }
        try {
            ...
            LOGGER.error(Messages.SVGExportWizard_7 + ex.getMessage(), ex);
            MessageBox mb = new MessageBox(Display.getDefault().getActiveShell(), SWT.ICON_ERROR | SWT.OK);
            mb.setText(Messages.SVGExportWizard_8);
            mb.setMessage(Messages.SVGExportWizard_9 + ex.getMessage());
            mb.open();
            return false;
        }
    }
    ...
    private boolean export() throws SVGExportException {
        ...
        if (outFile.exists()) {
            ...
            if (!outFile.canWrite() || outFile.isDirectory()) {
                // display error
                m_page.setErrorMessage(Messages.SVGExportWizard_12);
                return false;
            }
            boolean overwrite =
                MessageDialog.openQuestion(getShell(), Messages.SVGExportWizard_13,
                    Messages.SVGExportWizard_14 + Messages.SVGExportWizard_15);
            if (!overwrite) {
                return false;
            }
        }
