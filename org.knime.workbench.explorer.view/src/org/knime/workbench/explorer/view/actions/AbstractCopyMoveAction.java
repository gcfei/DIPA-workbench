...
public abstract class AbstractCopyMoveAction extends ExplorerAction {
    ...
    public AbstractCopyMoveAction(final ExplorerView viewer,
        ...
        m_cmd = m_performMove ? Messages.getString("AbstractCopyMoveAction.0") : Messages.getString("AbstractCopyMoveAction.1"); //$NON-NLS-1$ //$NON-NLS-2$
    }
    ...
    public void run() {
        if (m_sources == null) {
            ...
            if (m_sources == null || m_sources.isEmpty()
                ...
                String action = m_performMove ? Messages.getString("AbstractCopyMoveAction.2") : Messages.getString("AbstractCopyMoveAction.3"); //$NON-NLS-1$ //$NON-NLS-2$
                mb.setText(Messages.getString("AbstractCopyMoveAction.4") + action + Messages.getString("AbstractCopyMoveAction.5")); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        if (!m_success) {
            LOGGER.debug((m_performMove ? Messages.getString("AbstractCopyMoveAction.6") : Messages.getString("AbstractCopyMoveAction.7")) + Messages.getString("AbstractCopyMoveAction.8") + m_target.getFullName() + Messages.getString("AbstractCopyMoveAction.9")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        } else {
            LOGGER.debug(Messages.getString("AbstractCopyMoveAction.10") + (m_performMove ? Messages.getString("AbstractCopyMoveAction.11") : Messages.getString("AbstractCopyMoveAction.12")) + m_sources.size() + Messages.getString("AbstractCopyMoveAction.13") //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
                    + m_target.getFullName() + Messages.getString("AbstractCopyMoveAction.14")); //$NON-NLS-1$
        }
    }
    ...
    private void openTargetSelectionDialog() {
        ...
        dialog.setTitle(Messages.getString("AbstractCopyMoveAction.15")); //$NON-NLS-1$
        dialog.setDescription(
                Messages.getString("AbstractCopyMoveAction.16") //$NON-NLS-1$
                + (m_performMove ? Messages.getString("AbstractCopyMoveAction.17") : Messages.getString("AbstractCopyMoveAction.18")) //$NON-NLS-1$ //$NON-NLS-2$
                + Messages.getString("AbstractCopyMoveAction.19")); //$NON-NLS-1$
        dialog.setValidator(new Validator() {
            ...
            public String validateSelectionValue(final AbstractExplorerFileStore selection, final String name) {
                ...
                return isWFG ? null : Messages.getString("AbstractCopyMoveAction.20"); //$NON-NLS-1$
            }
        });
    ...
    protected boolean copyOrMove(final List<AbstractExplorerFileStore>
        ...
        if (isChildOf(m_target,
            ...
            String msg = Messages.getString("AbstractCopyMoveAction.21") + m_cmd.toLowerCase() + Messages.getString("AbstractCopyMoveAction.22") //$NON-NLS-1$ //$NON-NLS-2$
            + m_target.toString().replace("&", "&&") + Messages.getString("AbstractCopyMoveAction.25"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            MessageDialog.openError(Display.getDefault()
                    .getActiveShell(), m_cmd + Messages.getString("AbstractCopyMoveAction.26"), msg); //$NON-NLS-1$
            LOGGER.info(msg);
            return false;
        }
        ...
        for (final AbstractExplorerFileStore srcFS : srcFileStores) {
            ...
            if (destChecker.isAbort()) {
                LOGGER.info(m_cmd + Messages.getString("AbstractCopyMoveAction.27")); //$NON-NLS-1$
                return false;
            }
            if (destFS == null) {
                // the user skipped the operation or it is not allowed
                LOGGER.info(m_cmd + Messages.getString("AbstractCopyMoveAction.28") + srcFS.getMountIDWithFullPath() + Messages.getString("AbstractCopyMoveAction.29")); //$NON-NLS-1$ //$NON-NLS-2$
            }
        }
        ...
        for (AbstractContentProvider prov : overWrittenFlows.keySet()) {
            ...
            if (confirm != null && !confirm.get()) {
                LOGGER.info(Messages.getString("AbstractCopyMoveAction.30") + prov); //$NON-NLS-1$
                return false;
            }
        }
        ...
        if (m_performMove) {
            ...
            for (AbstractContentProvider acp : movedFlows.keySet()) {
                ...
                if (confirmed != null && !confirmed.get()) {
                    LOGGER.info(Messages.getString("AbstractCopyMoveAction.31") + acp); //$NON-NLS-1$
                    return false;
                }
            }
        }
        ...
        try {
            ...
            LOGGER.debug(Messages.getString("AbstractCopyMoveAction.32") + e.getMessage(), e); //$NON-NLS-1$
            result.add(new Status(IStatus.ERROR, ExplorerActivator.PLUGIN_ID,
                Messages.getString("AbstractCopyMoveAction.33") + e.getMessage(), e)); //$NON-NLS-1$
            success.set(false);
        } catch (InterruptedException e) {
            LOGGER.debug(m_cmd + Messages.getString("AbstractCopyMoveAction.34") + e.getMessage(), e); //$NON-NLS-1$
            result.add(new Status(IStatus.ERROR, ExplorerActivator.PLUGIN_ID, Messages.getString("AbstractCopyMoveAction.35") + e.getMessage(), e)); //$NON-NLS-1$
            success.set(false);
        } finally {
        ...
        if (result.size() > 1) {
            IStatus multiStatus = new MultiStatus(ExplorerActivator.PLUGIN_ID, IStatus.ERROR,
                result.toArray(new IStatus[0]), Messages.getString("AbstractCopyMoveAction.36") + m_cmd + Messages.getString("AbstractCopyMoveAction.37"), null); //$NON-NLS-1$ //$NON-NLS-2$
            ErrorDialog.openError(Display.getDefault().getActiveShell(), m_cmd + Messages.getString("AbstractCopyMoveAction.38"), //$NON-NLS-1$
                Messages.getString("AbstractCopyMoveAction.39"), multiStatus); //$NON-NLS-1$
            // Don't show it as failure if only some of the items could not be copied.
            success.set(true);
        } else if (result.size() == 1) {
            ErrorDialog.openError(Display.getDefault().getActiveShell(), m_cmd + Messages.getString("AbstractCopyMoveAction.40"), //$NON-NLS-1$
                Messages.getString("AbstractCopyMoveAction.41"), result.get(0)); //$NON-NLS-1$
        }
        return success.get();
    }
