...
public class TempExtractArchive {
    ...
    */
    ...
    protected void unpackWorkflowIntoLocalDir() throws Exception {
        ...
        if (m_source == null) {
            LOGGER.error(Messages.getString("TempExtractArchive.0")); //$NON-NLS-1$
            return;
        }
        ...
        LOGGER.debug(Messages.getString("TempExtractArchive.1") + element.getName() + Messages.getString("TempExtractArchive.2") //$NON-NLS-1$ //$NON-NLS-2$
    ...
    protected void runSyncInternal(final IProgressMonitor monitor) throws CoreException {
        ...
        try {
            ...
            LOGGER.error(Messages.getString("TempExtractArchive.3"), e); //$NON-NLS-1$
        }
        ...
        if (m_deleteSource) {
            if (!m_source.delete()) {
                LOGGER.error(Messages.getString("TempExtractArchive.4")); //$NON-NLS-1$
            }
        }
