...
public class NewWorkflowGroupWizard extends NewWorkflowWizard {
    ...
    protected void doFinish(final AbstractExplorerFileStore newItem,
        ...
        if (newItem.fetchInfo().exists()) {
            throwCoreException(Messages.getString("NewWorkflowGroupWizard.0") + newItem.getFullName() //$NON-NLS-1$
                    + Messages.getString("NewWorkflowGroupWizard.1"), null); //$NON-NLS-1$
        }

        if (!newItem.getParent().fetchInfo().exists()) {
            throwCoreException(Messages.getString("NewWorkflowGroupWizard.2") //$NON-NLS-1$
                    + Messages.getString("NewWorkflowGroupWizard.3") //$NON-NLS-1$
                    + Messages.getString("NewWorkflowGroupWizard.4"), null); //$NON-NLS-1$
        }
