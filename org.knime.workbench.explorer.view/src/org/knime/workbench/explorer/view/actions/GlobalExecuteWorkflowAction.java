...
public class GlobalExecuteWorkflowAction extends ExplorerAction {
    ...
    public static final String EXECUTEWF_ACTION_ID =
            "org.knime.workbench.explorer.action.execute-workflow"; //$NON-NLS-1$

    /**
     * @param viewer the associated tree viewer
     */
    public GlobalExecuteWorkflowAction(final ExplorerView viewer) {
        super(viewer, Messages.getString("GlobalExecuteWorkflowAction.1")); //$NON-NLS-1$
    ...
    public void run() {
        ...
        if (!(wfStore instanceof LocalExplorerFileStore)) {
            LOGGER.error(Messages.getString("GlobalExecuteWorkflowAction.2")); //$NON-NLS-1$
            return;
        }
        ...
        try {
            if (ExplorerFileSystemUtils
                ...
                LOGGER.info(Messages.getString("GlobalExecuteWorkflowAction.3") //$NON-NLS-1$
                        + Messages.getString("GlobalExecuteWorkflowAction.4") //$NON-NLS-1$
                        + Messages.getString("GlobalExecuteWorkflowAction.5")); //$NON-NLS-1$
    ...
    private void showCantExecuteLockMessage() {
        ...
        mb.setText(Messages.getString("GlobalExecuteWorkflowAction.6")); //$NON-NLS-1$
        mb.setMessage(Messages.getString("GlobalExecuteWorkflowAction.7") //$NON-NLS-1$
                + Messages.getString("GlobalExecuteWorkflowAction.8") //$NON-NLS-1$
                + Messages.getString("GlobalExecuteWorkflowAction.9")); //$NON-NLS-1$
        mb.open();
    }
}
