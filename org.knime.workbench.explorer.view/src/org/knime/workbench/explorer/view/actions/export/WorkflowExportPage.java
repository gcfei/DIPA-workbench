...
public class WorkflowExportPage extends WizardPage {
    ...
    private static final String[] FILTER_EXTENSION =
        {"*." + KNIMEConstants.KNIME_ARCHIVE_FILE_EXTENSION + ";*." + KNIMEConstants.KNIME_WORKFLOW_FILE_EXTENSION}; //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public WorkflowExportPage(final AbstractExplorerFileStore selection) {
        ...
        setTitle(Messages.getString("WorkflowExportPage.3")); //$NON-NLS-1$
        setDescription(Messages.getString("WorkflowExportPage.4")); //$NON-NLS-1$
    ...
    public void createControl(final Composite parent) {
        ...
        label.setText(Messages.getString("WorkflowExportPage.5")); //$NON-NLS-1$
        ...
        selectProjectButton.setText(Messages.getString("WorkflowExportPage.6")); //$NON-NLS-1$
        ...
        if (AbstractExplorerFileStore.isWorkflow(m_selection)) {
            m_fileLabel.setText(Messages.getString("WorkflowExportPage.7") + KNIMEConstants.KNIME_WORKFLOW_FILE_EXTENSION + "):"); //$NON-NLS-1$ //$NON-NLS-2$
        } else {
            m_fileLabel.setText(Messages.getString("WorkflowExportPage.9") + KNIMEConstants.KNIME_ARCHIVE_FILE_EXTENSION + "):"); //$NON-NLS-1$ //$NON-NLS-2$
        }
        ...
        selectExportFilebutton.setText(Messages.getString("WorkflowExportPage.11")); //$NON-NLS-1$
        ...
        group.setText(Messages.getString("WorkflowExportPage.12")); //$NON-NLS-1$
        ...
        m_excludeData.setText(Messages.getString("WorkflowExportPage.13")); //$NON-NLS-1$
    ...
    private void createTreeViewer(final Composite parent) {
        ...
        m_selAll.setText(Messages.getString("WorkflowExportPage.14")); //$NON-NLS-1$
        ...
        m_deselAll.setText(Messages.getString("WorkflowExportPage.15")); //$NON-NLS-1$
    ...
    private void updateFileName(final AbstractExplorerFileStore sel) {
        if (sel != null) {
            String fileName = sel.getName() + "."; //$NON-NLS-1$
            if (AbstractExplorerFileStore.isWorkflow(sel)) {
                fileName += KNIMEConstants.KNIME_WORKFLOW_FILE_EXTENSION;
                m_fileLabel
                    .setText(Messages.getString("WorkflowExportPage.17") + KNIMEConstants.KNIME_WORKFLOW_FILE_EXTENSION + "):"); //$NON-NLS-1$ //$NON-NLS-2$
            } else {
                fileName += KNIMEConstants.KNIME_ARCHIVE_FILE_EXTENSION;
                m_fileLabel
                    .setText(Messages.getString("WorkflowExportPage.19") + KNIMEConstants.KNIME_ARCHIVE_FILE_EXTENSION + "):"); //$NON-NLS-1$ //$NON-NLS-2$
            }
            if (sel.getFullName().equals("/")) { //$NON-NLS-1$
                fileName = "knime-export." + KNIMEConstants.KNIME_ARCHIVE_FILE_EXTENSION; //$NON-NLS-1$
            }
            File f = null;
            ...
            if (f == null) {
                ...
                f = new File(System.getProperty("user.home"), fileName); //$NON-NLS-1$
            }
            m_fileText.setText(f.getAbsolutePath());
        } else {
            m_fileText.setText(""); //$NON-NLS-1$
        }
    }
    ...
    private void handleFlowSelect() {
        ...
        dlg.setTitle(Messages.getString("WorkflowExportPage.25")); //$NON-NLS-1$
        dlg.setHeader(Messages.getString("WorkflowExportPage.26")); //$NON-NLS-1$
        dlg.setDescription(Messages.getString("WorkflowExportPage.27")); //$NON-NLS-1$
    ...
    private void containerSelectionChanged() {
        ...
        if (m_selection != null) {
            ...
            m_containerText.setText(""); //$NON-NLS-1$
        }
    ...
    private void handleExportFileBrowse() {
        ...
        fileDialog.setText(Messages.getString("WorkflowExportPage.29") //$NON-NLS-1$
                + Messages.getString("WorkflowExportPage.30")); //$NON-NLS-1$
        ...
        if (AbstractExplorerFileStore.isWorkflow(m_selection)) {
            if (!filePath.toLowerCase().endsWith("." + KNIMEConstants.KNIME_WORKFLOW_FILE_EXTENSION)) { //$NON-NLS-1$
                filePath += "." + KNIMEConstants.KNIME_WORKFLOW_FILE_EXTENSION; //$NON-NLS-1$
            }
        } else {
            if (!filePath.toLowerCase().endsWith("." + KNIMEConstants.KNIME_ARCHIVE_FILE_EXTENSION)) { //$NON-NLS-1$
                filePath += "." + KNIMEConstants.KNIME_ARCHIVE_FILE_EXTENSION; //$NON-NLS-1$
            }
        }
    ...
    private String getStatus() {
        if (m_selection == null) {
            return Messages.getString("WorkflowExportPage.35"); //$NON-NLS-1$
        }
        String fileName = getFileName();
        if (fileName.length() == 0 || fileName.endsWith(File.separator)) {
            return Messages.getString("WorkflowExportPage.36"); //$NON-NLS-1$
        }
        if (m_treeViewer.getTree().isVisible()
                && m_treeViewer.getCheckedElements().length == 0) {
            return Messages.getString("WorkflowExportPage.37"); //$NON-NLS-1$
        }
        Collection<AbstractExplorerFileStore> sel = getElementsToExport();
        if (sel.isEmpty()) {
            return Messages.getString("WorkflowExportPage.38"); //$NON-NLS-1$
        } else {
            return null;
        }
    }
    ...
    public Collection<AbstractExplorerFileStore> getElementsToExport() {
        ...
        if (AbstractExplorerFileStore.isWorkflowGroup(m_selection)) {
            ...
            if (!"/".equals(m_selection.getFullName())) { //$NON-NLS-1$
                elements.add(m_selection);
            }
    ...
    private static final String KEY_LOC = "destination-location"; //$NON-NLS-1$
