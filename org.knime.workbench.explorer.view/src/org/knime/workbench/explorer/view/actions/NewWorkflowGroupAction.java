...
public class NewWorkflowGroupAction extends ExplorerAction {
    ...
    private static final ImageDescriptor ICON = KNIMEUIPlugin
            .imageDescriptorFromPlugin(KNIMEUIPlugin.PLUGIN_ID,
                    "icons/wf_group_new.png"); //$NON-NLS-1$

    /** ID of the global new workflow group action in the explorer menu. */
    public static final String ID = "org.knime.explorer.view.actions.newgroup"; //$NON-NLS-1$
    ...
    public NewWorkflowGroupAction(final ExplorerView viewer) {
        super(viewer, Messages.getString("NewWorkflowGroupAction.2")); //$NON-NLS-1$
    ...
    public boolean isEnabled() {
     // only enabled if exactly on file is selected
        ...
        if (fileInfo.isWorkflowGroup()) {
            ...
            if (fileInfo.isReservedSystemItem()) {
                return false;
            }
        }
    ...
    public void run() {
        Map<AbstractContentProvider, List<AbstractExplorerFileStore>> sel =
                getSelectedFiles();
        if (sel.size() != 1) {
            LOGGER.info(Messages.getString("NewWorkflowGroupAction.3") //$NON-NLS-1$
                    + Messages.getString("NewWorkflowGroupAction.4")); //$NON-NLS-1$
            return;
        }
        ...
        if (list == null || list.size() != 1) {
            ...
            LOGGER.debug(Messages.getString("NewWorkflowGroupAction.5")); //$NON-NLS-1$
            return;
        }
        if (!(list.get(0).getMountID().equals(provider.getMountID()))) {
            LOGGER.error(Messages.getString("NewWorkflowGroupAction.6")); //$NON-NLS-1$
            return;
        }
        NewWorkflowWizard newWiz = new NewWorkflowGroupWizard();
