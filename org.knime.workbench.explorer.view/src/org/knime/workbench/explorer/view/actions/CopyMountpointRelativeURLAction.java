...
public class CopyMountpointRelativeURLAction extends ExplorerAction {
    ...
    public static final String URLCOPY_ACTION_ID = "org.knime.workbench.explorer.action.copy-url"; //$NON-NLS-1$
    ...
    public CopyMountpointRelativeURLAction(final ExplorerView viewer, final Clipboard cb) {
        super(viewer, Messages.getString("CopyMountpointRelativeURLAction.1")); //$NON-NLS-1$
        m_cb = cb;
        setToolTipText(Messages.getString("CopyMountpointRelativeURLAction.2")); //$NON-NLS-1$
    ...
    @Override
    public String getId() {
        return "org.knime.workbench.explorer.action.copy-mp-relative-url"; //$NON-NLS-1$
    }
    ...
    public void run() {
        ...
        while (i.hasNext()) {
            ...
            if (fs != null) {
                ...
                try {
                    uri =
                        new URI(uri.getScheme(), uri.getUserInfo(), "knime.mountpoint", -1, uri.getPath(), //$NON-NLS-1$
                            uri.getQuery(), uri.getFragment());
                    url.append(uri.toString());
                } catch (URISyntaxException ex) {
                    // cannot happen because the URI was valid before.
                    url.append("<null>"); //$NON-NLS-1$
                }
            } else {
                url.append("<null>"); //$NON-NLS-1$
            }
