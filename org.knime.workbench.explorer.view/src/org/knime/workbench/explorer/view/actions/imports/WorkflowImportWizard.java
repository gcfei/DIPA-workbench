 ...
 *
 * History
 *   6.06.2011 Peter Ohl, KNIME AG, Zurich, Switzerland
 */
...
public class WorkflowImportWizard extends Wizard {
    ...
    public WorkflowImportWizard() {
        super();
        setWindowTitle(Messages.getString("WorkflowImportWizard.0")); //$NON-NLS-1$
        ...
        if (destination == null) {
            ...
            for (AbstractContentProvider prov : mountedContent.values()) {
                if (!prov.isRemote() && prov.isWritable()) {
                    ...
                    if (prov.getMountID().equals("LOCAL")) { //$NON-NLS-1$
    ...
    public boolean createWorkflows() {
       ...
       final Collection<IWorkflowImportElement> workflows = m_importPage
        ...
        try {
            ...
            String message = Messages.getString("WorkflowImportWizard.2"); //$NON-NLS-1$
            IStatus status;
            ...
            String message = Messages.getString("WorkflowImportWizard.3"); //$NON-NLS-1$
    ...
    private void selectImportedElementsInExplorerView(final ArrayList<IWorkflowImportElement> topLevelElements) {
        ...
        IViewPart viewPart = activePage.findView("org.knime.workbench.explorer.view"); //$NON-NLS-1$
        if (viewPart == null) {
            return;
        }
