...
public class GlobalConfigureWorkflowAction extends ExplorerAction {
    ...
    public static final String CONFIGUREWF_ACTION_ID =
            "org.knime.workbench.explorer.action.configure-workflow"; //$NON-NLS-1$

    /**
     * @param viewer the associated tree viewer
     */
    public GlobalConfigureWorkflowAction(final ExplorerView viewer) {
        super(viewer, Messages.getString("GlobalConfigureWorkflowAction.1")); //$NON-NLS-1$
    ...
    public void run() {
        ...
        if (!(wfStore instanceof LocalExplorerFileStore)) {
            LOGGER.error(Messages.getString("GlobalConfigureWorkflowAction.2")); //$NON-NLS-1$
            return;
        }
        try {
            if (ExplorerFileSystemUtils
                ...
                LOGGER.info(Messages.getString("GlobalConfigureWorkflowAction.3") //$NON-NLS-1$
                        + Messages.getString("GlobalConfigureWorkflowAction.4")); //$NON-NLS-1$
                        ...
                        public void run() {
                            MessageDialog
                                    .openError(
                                            Display.getDefault()
                                                    .getActiveShell(),
                                            Messages.getString("GlobalConfigureWorkflowAction.5"), //$NON-NLS-1$
                                            Messages.getString("GlobalConfigureWorkflowAction.6") //$NON-NLS-1$
                                                    + Messages.getString("GlobalConfigureWorkflowAction.7") //$NON-NLS-1$
                                                    + nce.getMessage());
                        }
                    });
                }
    ...
    private void showCantConfigureLockMessage() {
        ...
        mb.setText(Messages.getString("GlobalConfigureWorkflowAction.8")); //$NON-NLS-1$
        mb.setMessage(Messages.getString("GlobalConfigureWorkflowAction.9") //$NON-NLS-1$
                + Messages.getString("GlobalConfigureWorkflowAction.10")); //$NON-NLS-1$
        mb.open();
    }
}
