...
public class NewWorkflowAction extends ExplorerAction {
    ...
    public static final String ID =
            "org.knime.explorer.view.actions.newflow"; //$NON-NLS-1$
    ...
    private static final ImageDescriptor ICON = ExplorerActivator
            .imageDescriptorFromPlugin(ExplorerActivator.PLUGIN_ID,
                    "icons/new_dipa16.png");; //$NON-NLS-1$

    /**
     * @param viewer underlying viewer
     */
    public NewWorkflowAction(final ExplorerView viewer) {
        //super(viewer, Messages.getString("NewWorkflowAction.2")); //$NON-NLS-1$
        super(viewer, Messages.getString("NewWorkflowAction.2")); //$NON-NLS-1$
    ...
    public void run() {
        Map<AbstractContentProvider, List<AbstractExplorerFileStore>> sel =
                getSelectedFiles();
        if (sel.size() != 1) {
            LOGGER.info(Messages.getString("NewWorkflowAction.3") //$NON-NLS-1$
                    + Messages.getString("NewWorkflowAction.4")); //$NON-NLS-1$
            return;
        }
        ...
        if (list == null || list.size() != 1) {
            ...
            LOGGER.debug(Messages.getString("NewWorkflowAction.5")); //$NON-NLS-1$
            return;
        }
        if (!(list.get(0).getMountID().equals(provider.getMountID()))) {
            LOGGER.error(Messages.getString("NewWorkflowAction.6")); //$NON-NLS-1$
            return;
        }
