...
public class GlobalResetWorkflowAction extends ExplorerAction {
    ...
    public static final String RESETACTION_ID =
            "org.knime.workbench.explorer.action.reset"; //$NON-NLS-1$

    /**
     * @param viewer the associated tree viewer
     */
    public GlobalResetWorkflowAction(final ExplorerView viewer) {
        super(viewer, Messages.getString("GlobalResetWorkflowAction.1")); //$NON-NLS-1$
        setToolTipText(Messages.getString("GlobalResetWorkflowAction.2")); //$NON-NLS-1$
    ...
    public void run() {
        ...
        if (!(wfStore instanceof LocalExplorerFileStore)) {
            LOGGER.error(Messages.getString("GlobalResetWorkflowAction.3")); //$NON-NLS-1$
            return;
        }
        WorkflowManager wfm = getWorkflow();
