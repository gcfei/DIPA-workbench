...
public abstract class AbstractContentProvider extends LabelProvider implements
    ...
    static {
        HIDDEN_FILENAMES.add("workflowset.meta"); //$NON-NLS-1$
    }
    ...
    public AbstractContentProvider(
        ...
        if (myCreator == null) {
            throw new NullPointerException(
                    Messages.getString("AbstractContentProvider.1")); //$NON-NLS-1$
        }
        if (id == null || id.isEmpty()) {
            throw new NullPointerException(
                    Messages.getString("AbstractContentProvider.2")); //$NON-NLS-1$
        }
        m_creator = myCreator;
        m_id = id;
    }
    ...
    public static final boolean isHiddenFile(final String fileName) {
        if (fileName != null) {
            return fileName.startsWith(".") || HIDDEN_FILENAMES.contains(fileName); //$NON-NLS-1$
        }
        return false;
    }
    ...
    public static String getRelativePath(final File file, final File root) {
        ...
        for (String s : segments) {
            if (!s.isEmpty()) {
                path.append("/").append(s); //$NON-NLS-1$
            }
        }
    ...
    public boolean saveMetaNodeTemplate(final WorkflowManager metaNode,
        ...
        if (new FileStoreNameValidator().isValid(uniqueName) != null) {
            InputDialog dialog = new InputDialog(Display.getDefault().getActiveShell(), "Metanode rename", //$NON-NLS-1$
                    Messages.getString("AbstractContentProvider.8") + uniqueName + Messages.getString("AbstractContentProvider.9") //$NON-NLS-1$ //$NON-NLS-2$
                    + Messages.getString("AbstractContentProvider.10") //$NON-NLS-1$
                    + Messages.getString("AbstractContentProvider.11"), uniqueName, //$NON-NLS-1$
            ...
            DestinationChecker<AbstractExplorerFileStore,
                AbstractExplorerFileStore> dc = new DestinationChecker
                    <AbstractExplorerFileStore, AbstractExplorerFileStore>(
                            Display.getDefault().getActiveShell(), "create template", false, false); //$NON-NLS-1$
        ...
        try {
            if (directory == null) {
                LOGGER.error(Messages.getString("AbstractContentProvider.13") + templateLoc //$NON-NLS-1$
                        + Messages.getString("AbstractContentProvider.14") + Messages.getString("AbstractContentProvider.15") //$NON-NLS-1$ //$NON-NLS-2$
                        + toString() + "\""); //$NON-NLS-1$
                return false;
            }
            if (directory.exists()) {
                if (!directory.isDirectory()) {
                    LOGGER.error(Messages.getString("AbstractContentProvider.17")  //$NON-NLS-1$
                            + Messages.getString("AbstractContentProvider.18")); //$NON-NLS-1$
                    return false;
                }
                if (!directory.canWrite()) {
                    MessageDialog.openWarning(Display.getDefault().getActiveShell(), "No write permission", //$NON-NLS-1$
                            Messages.getString("AbstractContentProvider.20") //$NON-NLS-1$
                                    + Messages.getString("AbstractContentProvider.21") //$NON-NLS-1$
                                    + mountIDWithFullPath + "\""); //$NON-NLS-1$
                    return false;
                }
            }

            if (!metaTemplateDropPrepareForSave(templateLoc, directory, isOverwrite, false)) {
                LOGGER.debug(Messages.getString("AbstractContentProvider.23")); //$NON-NLS-1$
                return false;
            }
            try {
                ...
                String error = Messages.getString("AbstractContentProvider.24") + e.getMessage(); //$NON-NLS-1$
                LOGGER.warn(error, e);
                MessageDialog.openError(Display.getDefault().getActiveShell(), Messages.getString("AbstractContentProvider.25"), //$NON-NLS-1$
                        error);
            }
    ...
    public static URI createMetanodeLinkUri(final NodeContainerParent node,
        ...
        if (linkType.equals(LinkType.Absolute)) {
            ...
            if (templateMountpointRoot == null) {
                LOGGER.warn(Messages.getString("AbstractContentProvider.26")); //$NON-NLS-1$
                return originalUri;
            }
            ...
            if (workflowMountpointRoot == null) {
                LOGGER.warn(Messages.getString("AbstractContentProvider.27")); //$NON-NLS-1$
                return originalUri;
            }

            if (!templateMountpointRoot.equals(workflowMountpointRoot)) {
                LOGGER.warn(Messages.getString("AbstractContentProvider.28")); //$NON-NLS-1$
                return originalUri;
            }

            if (linkType.equals(LinkType.MountpointRelative)) {
                return new URI(originalUri.getScheme(), originalUri.getUserInfo(), "knime.mountpoint", -1, //$NON-NLS-1$
                    originalUri.getPath(), originalUri.getQuery(), originalUri.getFragment());
            } else if (linkType.equals(LinkType.WorkflowRelative)) {
                String[] templatePathParts = templateLocation.toLocalFile().getAbsolutePath().split("[/\\\\]"); //$NON-NLS-1$
                String[] workflowPathParts = wfc.getCurrentLocation().getAbsolutePath().split("[/\\\\]"); //$NON-NLS-1$
                ...
                for (int i = indexWhereDifferent; i < workflowPathParts.length; i++) {
                    relPath.append("/.."); //$NON-NLS-1$
                }
                for (int i = indexWhereDifferent; i < templatePathParts.length; i++) {
                    relPath.append('/').append(templatePathParts[i]);
                }
                return new URI(ExplorerFileSystem.SCHEME, "knime.workflow", relPath.toString(), null); //$NON-NLS-1$
            } else {
                throw new IllegalArgumentException(Messages.getString("AbstractContentProvider.34") + linkType); //$NON-NLS-1$
            }
        }
    }
    ...
    protected File metaTemplateDropGetTempDir(
        ...
        try {
            ...
            throw new IllegalStateException(Messages.getString("AbstractContentProvider.35") //$NON-NLS-1$
                    + Messages.getString("AbstractContentProvider.36")); //$NON-NLS-1$
        }
        if (result == null) {
            throw new IllegalStateException(Messages.getString("AbstractContentProvider.37") //$NON-NLS-1$
                    + Messages.getString("AbstractContentProvider.38")); //$NON-NLS-1$
        }
        return result;
    }
            ...
            while (!VMFileLocker.lockForVM(tmpDir)) {
                MessageDialog dialog =
                        new MessageDialog(
                                ...
                                Messages.getString("AbstractContentProvider.39"), null, //$NON-NLS-1$
                                Messages.getString("AbstractContentProvider.40") //$NON-NLS-1$
                                        + target.getMountIDWithFullPath()
                                        + Messages.getString("AbstractContentProvider.41"), //$NON-NLS-1$
                                MessageDialog.QUESTION, new String[]{
                                        "&Try again", "&Cancel"}, 0); //$NON-NLS-1$ //$NON-NLS-2$
                if (dialog.open() == 0) {
                    continue; // next try
                } else {
                    return false; // abort
    ...
    private static LinkType promptLinkMetaNodeTemplate(final String oldName, final String newName,
        ...
        if (MessageDialogWithToggle.NEVER.equals(linkPrefs)) {
            ...
            String msg = Messages.getString("AbstractContentProvider.44") + (isSubnode ? Messages.getString("AbstractContentProvider.45") : Messages.getString("AbstractContentProvider.46")) + "?"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            if (!oldName.equals(newName)) {
                msg = msg + Messages.getString("AbstractContentProvider.48") + newName + Messages.getString("AbstractContentProvider.49"); //$NON-NLS-1$ //$NON-NLS-2$
            }
            final String title = Messages.getString("AbstractContentProvider.50") + (isSubnode ? Messages.getString("AbstractContentProvider.51") : Messages.getString("AbstractContentProvider.52")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    ...
    private static final class LinkPrompt extends MessageDialog {
        ...
        protected Control createCustomArea(final Composite parent) {
            ...
            l1.setText(Messages.getString("AbstractContentProvider.53")); //$NON-NLS-1$

            m_absoluteLink = new Button(group, SWT.RADIO);
            m_absoluteLink.setText(Messages.getString("AbstractContentProvider.54")); //$NON-NLS-1$
            m_absoluteLink.setLayoutData(data);
            m_absoluteLink.setToolTipText(Messages.getString("AbstractContentProvider.55") //$NON-NLS-1$
                + Messages.getString("AbstractContentProvider.56")); //$NON-NLS-1$
            ...
            m_mountpointRelativeLink.setText(Messages.getString("AbstractContentProvider.57")); //$NON-NLS-1$
            m_mountpointRelativeLink.setToolTipText(Messages.getString("AbstractContentProvider.58") //$NON-NLS-1$
                + Messages.getString("AbstractContentProvider.59")); //$NON-NLS-1$
            ...
            m_workflowRelativeLink.setText(Messages.getString("AbstractContentProvider.60")); //$NON-NLS-1$
            m_workflowRelativeLink.setToolTipText(Messages.getString("AbstractContentProvider.61")); //$NON-NLS-1$
            ...
            m_noLink.setText(Messages.getString("AbstractContentProvider.62")); //$NON-NLS-1$
            m_noLink.setToolTipText(Messages.getString("AbstractContentProvider.63")); //$NON-NLS-1$
    ...
    public boolean saveSubNodeTemplate(final SubNodeContainer subNode, final AbstractExplorerFileStore target,
        ...
        String uniqueName = originalName;
        if (new FileStoreNameValidator().isValid(uniqueName) != null) {
            InputDialog dialog = new InputDialog(Display.getDefault().getActiveShell(), Messages.getString("AbstractContentProvider.64"), //$NON-NLS-1$
                    Messages.getString("AbstractContentProvider.65") + uniqueName + Messages.getString("AbstractContentProvider.66") //$NON-NLS-1$ //$NON-NLS-2$
                    + Messages.getString("AbstractContentProvider.67") //$NON-NLS-1$
                    + Messages.getString("AbstractContentProvider.68"), uniqueName, //$NON-NLS-1$
                    new FileStoreNameValidator());
            dialog.setBlockOnOpen(true);
            if (dialog.open() == Window.CANCEL) {
                return false;
        ...
        if (doesTargetExist) {
            DestinationChecker<AbstractExplorerFileStore,
                AbstractExplorerFileStore> dc = new DestinationChecker
                    <AbstractExplorerFileStore, AbstractExplorerFileStore>(
                            Display.getDefault().getActiveShell(), Messages.getString("AbstractContentProvider.69"), false, false); //$NON-NLS-1$
            dc.setIsOverwriteEnabled(overwriteOK);
            dc.setIsOverwriteDefault(overwriteOK);
        ...
        try {
            if (directory == null) {
                LOGGER.error(Messages.getString("AbstractContentProvider.70") + templateLoc //$NON-NLS-1$
                        + Messages.getString("AbstractContentProvider.71") + Messages.getString("AbstractContentProvider.72") //$NON-NLS-1$ //$NON-NLS-2$
                        + toString() + "\""); //$NON-NLS-1$
                return false;
            }
            if (directory.exists()) {
                if (!directory.isDirectory()) {
                    LOGGER.error(Messages.getString("AbstractContentProvider.74") //$NON-NLS-1$
                            + Messages.getString("AbstractContentProvider.75")); //$NON-NLS-1$
                    return false;
                }
                if (!directory.canWrite()) {
                    MessageDialog.openWarning(Display.getDefault().getActiveShell(), Messages.getString("AbstractContentProvider.76"), //$NON-NLS-1$
                            Messages.getString("AbstractContentProvider.77") //$NON-NLS-1$
                                    + Messages.getString("AbstractContentProvider.78") //$NON-NLS-1$
                                    + mountIDWithFullPath + "\""); //$NON-NLS-1$
                    return false;
                }
            }
            ...
            if (!metaTemplateDropPrepareForSave(templateLoc, directory, isOverwrite, true)) {
                LOGGER.debug(Messages.getString("AbstractContentProvider.80")); //$NON-NLS-1$
                return false;
            }
            ...
            IRunnableWithProgress runnable = pm -> {
                pm.setTaskName(Messages.getString("AbstractContentProvider.81")); //$NON-NLS-1$
                try {
                    MetaNodeTemplateInformation template =
            ...
            String error = Messages.getString("AbstractContentProvider.82") + e.getTargetException().getMessage(); //$NON-NLS-1$
            LOGGER.warn(error, e.getTargetException());
            MessageDialog.openError(Display.getDefault().getActiveShell(), Messages.getString("AbstractContentProvider.83"), //$NON-NLS-1$
                error);
    ...
    protected String checkOpenEditors(
        ...
        if (msg != null) {
            ...
            mb.setText(Messages.getString("AbstractContentProvider.84")); //$NON-NLS-1$
            mb.setMessage(msg);
            mb.open();
        }
        return msg;
