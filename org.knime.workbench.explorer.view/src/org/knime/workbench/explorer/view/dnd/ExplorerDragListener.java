...
public class ExplorerDragListener implements DragSourceListener {
    ...
    public void dragStart(final DragSourceEvent event) {
        ...
        if (providers == null) {
            ...
            LOGGER.warn(Messages.getString("ExplorerDragListener.0")); //$NON-NLS-1$
            event.doit = false;
        } else {
            ...
            for (Map.Entry<AbstractContentProvider,
                ...
                if (!provider.dragStart(entry.getValue())) {
                    ...
                    event.doit = false;
                    LOGGER.debug(Messages.getString("ExplorerDragListener.1") + provider.getMountID() //$NON-NLS-1$
                            + Messages.getString("ExplorerDragListener.2")); //$NON-NLS-1$
                    return;
                }
            }
            event.doit = true;
