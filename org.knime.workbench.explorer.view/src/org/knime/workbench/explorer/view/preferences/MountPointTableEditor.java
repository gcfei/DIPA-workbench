...
public class MountPointTableEditor extends FieldEditor {
    ...
    public MountPointTableEditor(final Composite parent) {
        init(PreferenceConstants.P_EXPLORER_MOUNT_POINT_XML, Messages.getString("MountPointTableEditor.0")); //$NON-NLS-1$
    ...
    private TableViewer getTableControl(final Composite parent) {
        ...
        mountCol.setText(Messages.getString("MountPointTableEditor.1")); //$NON-NLS-1$
        mountCol.setWidth(150);
        mountCol.setResizable(true);

        TableColumn contentCol = new TableColumn(m_table, SWT.NONE);
        contentCol.setText(Messages.getString("MountPointTableEditor.2")); //$NON-NLS-1$
        contentCol.setWidth(200);
        contentCol.setResizable(true);

        TableColumn typeCol = new TableColumn(m_table, SWT.NONE);
        typeCol.setText(Messages.getString("MountPointTableEditor.3")); //$NON-NLS-1$
    ...
    private void createButtons(final Composite box) {
        m_addButton = createPushButton(box, Messages.getString("MountPointTableEditor.4")); //$NON-NLS-1$
        m_editButton = createPushButton(box, Messages.getString("MountPointTableEditor.5")); //$NON-NLS-1$
        m_removeButton = createPushButton(box, Messages.getString("MountPointTableEditor.6")); //$NON-NLS-1$
        m_upButton = createPushButton(box, Messages.getString("MountPointTableEditor.7")); //$NON-NLS-1$
        m_downButton = createPushButton(box, Messages.getString("MountPointTableEditor.8")); //$NON-NLS-1$
    }
    ...
    protected void doLoad() {
        try {
            ...
            LOGGER.error(Messages.getString("MountPointTableEditor.9") + e.getMessage(), e); //$NON-NLS-1$
        }
    }
    ...
    protected void doStore() {
        ...
        if (!m_removedMountPointNames.isEmpty()) {
            try {
                ...
                LOGGER.error(Messages.getString("MountPointTableEditor.10") + e.getMessage(), e); //$NON-NLS-1$
            }
        }
