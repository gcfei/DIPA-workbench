...
public class OverwriteRenameDialog extends Dialog {
    ...
    */
    ...
    protected void configureShell(final Shell newShell) {
        ...
        newShell.setText(Messages.getString("OverwriteRenameDialog.1")); //$NON-NLS-1$
    }
    ...
    private void createHeader(final Composite parent) {
        ...
        exec.setText(Messages.getString("OverwriteRenameDialog.2")); //$NON-NLS-1$
        ...
        txt.setText(Messages.getString("OverwriteRenameDialog.3") + m_destination.getName() //$NON-NLS-1$
                + Messages.getString("OverwriteRenameDialog.4") //$NON-NLS-1$
                + Messages.getString("OverwriteRenameDialog.5")); //$NON-NLS-1$
    ...
    private void createTextPanel(final Composite parent) {
        ...
        border.setText(Messages.getString("OverwriteRenameDialog.6")); //$NON-NLS-1$
        ...
        m_overwriteGUI.setText(Messages.getString("OverwriteRenameDialog.7")); //$NON-NLS-1$
        ...
        if (!m_canWriteDestination) {
            ...
            m_overwriteGUI.setToolTipText(Messages.getString("OverwriteRenameDialog.8") //$NON-NLS-1$
                    + Messages.getString("OverwriteRenameDialog.9")); //$NON-NLS-1$
        }
        ...
        m_renameGUI.setText(Messages.getString("OverwriteRenameDialog.10")); //$NON-NLS-1$
    ...
    protected void createButtonsForButtonBar(final Composite parent) {
        if (m_multiple) {
            createButton(parent, IDialogConstants.YES_TO_ALL_ID, Messages.getString("OverwriteRenameDialog.12"), //$NON-NLS-1$
                    false);
        }
    ...
    private void setSelectedAndValidate(final Button choice) {
        String errMsg = Messages.getString("OverwriteRenameDialog.14"); //$NON-NLS-1$
        ...
        try {
            if (m_renameGUI.getSelection()) {
                if (m_newName.isEmpty()) {
                    errMsg = Messages.getString("OverwriteRenameDialog.15"); //$NON-NLS-1$
                ...
                if (dest.fetchInfo().exists()) {
                    errMsg = Messages.getString("OverwriteRenameDialog.16"); //$NON-NLS-1$
                    return;
                } else if (m_forbiddenStores.contains(dest)) {
                    errMsg = Messages.getString("OverwriteRenameDialog.17") //$NON-NLS-1$
                        + Messages.getString("OverwriteRenameDialog.18"); //$NON-NLS-1$
                    return;
                }
            }
            errMsg = null;
    ...
    public static String getAlternativeName(
        ...
        while (child.fetchInfo().exists() || (forbiddenStores != null
            ...
            newName = origName + "(" + (++cnt) + ")" + extension; //$NON-NLS-1$ //$NON-NLS-2$
