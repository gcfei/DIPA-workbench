...
public class GlobalCancelWorkflowExecutionAction extends ExplorerAction {
    ...
    public static final String CANCELACTION_ID =
            "org.knime.workbench.explorer.action.cancel"; //$NON-NLS-1$

    /**
     * @param viewer the associated tree viewer
     */
    public GlobalCancelWorkflowExecutionAction(final ExplorerView viewer) {
        super(viewer, Messages.getString("GlobalCancelWorkflowExecutionAction.1")); //$NON-NLS-1$
        setToolTipText(Messages.getString("GlobalCancelWorkflowExecutionAction.2")); //$NON-NLS-1$
    ...
    public void run() {
        ...
        if (!(wfStore instanceof LocalExplorerFileStore)) {
            LOGGER.error(Messages.getString("GlobalCancelWorkflowExecutionAction.3")); //$NON-NLS-1$
            return;
        }
        WorkflowManager wfm = getWorkflow();
