...
public final class CopyMove {
    ...
    public CopyMoveResult run(final IProgressMonitor monitor) {
        ...
        monitor.beginTask(cmd + " " + numFiles + Messages.getString("CopyMove.1") + m_target.getFullName(), numFiles); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        for (final Map.Entry<AbstractExplorerFileStore, AbstractExplorerFileStore> entry : destCheckerMappings
            ...
            String operation = cmd + " " + srcFS.getMountIDWithFullPath() + Messages.getString("CopyMove.3") + destFS.getFullName(); //$NON-NLS-1$ //$NON-NLS-2$
            monitor.subTask(operation);
            LOGGER.debug(operation);
            try {
                if (m_notOverwritableDest.contains(destFS)) {
                    throw new UnsupportedOperationException(Messages.getString("CopyMove.4") + destFS.getFullName() //$NON-NLS-1$
                        + Messages.getString("CopyMove.5")); //$NON-NLS-1$
                }
                ...
                if (isOverwritten && (srcFSInfo.isWorkflowGroup() != destFS.fetchInfo().isWorkflowGroup())) {
                    ...
                    if (srcFSInfo.isWorkflowGroup()) {
                        msg = Messages.getString("CopyMove.6") + destFS.getFullName() + Messages.getString("CopyMove.7") //$NON-NLS-1$ //$NON-NLS-2$
                            + Messages.getString("CopyMove.8") + Messages.getString("CopyMove.9"); //$NON-NLS-1$ //$NON-NLS-2$
                    } else {
                        msg = Messages.getString("CopyMove.10") + destFS.getFullName() + Messages.getString("CopyMove.11") //$NON-NLS-1$ //$NON-NLS-2$
                            + Messages.getString("CopyMove.12") + Messages.getString("CopyMove.13"); //$NON-NLS-1$ //$NON-NLS-2$
                    }
                ...
                if (srcFSInfo.isWorkflowTemplate() && !destFS.getContentProvider().canHostWorkflowTemplate(srcFS)) {
                    throw new UnsupportedOperationException(Messages.getString("CopyMove.14") + cmd + Messages.getString("CopyMove.15") //$NON-NLS-1$ //$NON-NLS-2$
                        + srcFS.getFullName() + Messages.getString("CopyMove.16") + destFS.getMountID() + "." + Messages.getString("CopyMove.18")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                }
                ...
                if (!isSrcRemote && isDstRemote) { // upload
                    ...
                    CheckUtils.checkState(!m_excludeDataInWorkflows, Messages.getString("CopyMove.19")); //$NON-NLS-1$
                    ...
                    CheckUtils.checkState(!m_excludeDataInWorkflows, Messages.getString("CopyMove.20")); //$NON-NLS-1$
                    if (m_performMove) {
                ...
                LOGGER.debug(cmd + Messages.getString("CopyMove.21") + e.getStatus().getMessage(), e); //$NON-NLS-1$
                statusList.add(e.getStatus());
                success = false;
                processedTargets.remove(destFS);
            } catch (UnsupportedOperationException e) {
                // illegal operation
                LOGGER.debug(cmd + Messages.getString("CopyMove.22") + e.getMessage()); //$NON-NLS-1$
                statusList.add(new Status(IStatus.WARNING, ExplorerActivator.PLUGIN_ID, e.getMessage()));
                success = true;
                processedTargets.remove(destFS);
            } catch (Exception e) {
                LOGGER.debug(cmd + Messages.getString("CopyMove.23") + e.getMessage(), e); //$NON-NLS-1$
    ...
    private void scheduleDeletionOfRemainingWorkflowGroups(
        ...
        ExplorerJob job = new ExplorerJob(Messages.getString("CopyMove.24")) { //$NON-NLS-1$
    ...
    private String cmdAsTextual() {
        return m_performMove ? Messages.getString("CopyMove.25") : Messages.getString("CopyMove.26"); //$NON-NLS-1$ //$NON-NLS-2$
    }
    ...
    private ExplorerJob scheduleLocalCopyOrMove(final AbstractExplorerFileStore source,
        ...
        ExplorerJob job = new ExplorerJob(
            cmdAsTextual() + Messages.getString("CopyMove.27") + source.getMountIDWithFullPath() + Messages.getString("CopyMove.28") + destination.getMountIDWithFullPath()) { //$NON-NLS-1$ //$NON-NLS-2$
