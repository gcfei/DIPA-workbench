...
public class ExplorerURIDropUtil {
    ...
    public static boolean validateDrop(final String uri, final AbstractExplorerFileStore target,
        ...
        if (uri != null && uri.startsWith("http")) { //$NON-NLS-1$
    ...
    public static boolean performDrop(final String uri, final ExplorerView view,
        ...
        try {
            ...
            PlatformUI.getWorkbench().getProgressService().busyCursorWhile((monitor) -> {
                monitor.beginTask(Messages.getString("ExplorerURIDropUtil.2"), 100); //$NON-NLS-1$
                try {
                    Optional<EntityImport> entityImport =
                        URIImporterFinder.getInstance().createEntityImportFor(knimeURI);
                    if (!entityImport.isPresent()) {
                        LOGGER.warn(Messages.getString("ExplorerURIDropUtil.3") + knimeURI + Messages.getString("ExplorerURIDropUtil.4")); //$NON-NLS-1$ //$NON-NLS-2$
                        return;
                    }
                    ...
                    tmpDir.set(FileUtil.createTempDir("download")); //$NON-NLS-1$
                    ...
                    if (objType == RepoObjectType.Workflow) {
                        fileExt = "." + KNIMEConstants.KNIME_WORKFLOW_FILE_EXTENSION; //$NON-NLS-1$
                    } else if (objType == RepoObjectType.WorkflowGroup) {
                        fileExt = "." + KNIMEConstants.KNIME_ARCHIVE_FILE_EXTENSION; //$NON-NLS-1$
                    }

                    file.set(new File(tmpDir.get(), objImport.getName() + fileExt));
                    if (!tmpFile.renameTo(file.get())) {
                        LOGGER.warn(Messages.getString("ExplorerURIDropUtil.9") + file.get() + Messages.getString("ExplorerURIDropUtil.10") //$NON-NLS-1$ //$NON-NLS-2$
                            + file.get() + "'"); //$NON-NLS-1$
                        return;
                    }
                } catch (IOException e) {
                    LOGGER.warn(Messages.getString("ExplorerURIDropUtil.12") + knimeURI + Messages.getString("ExplorerURIDropUtil.13"), e); //$NON-NLS-1$ //$NON-NLS-2$
                    return;
                }
            });
            if (file.get() == null) {
            ...
            if (!file.get().delete() || !tmpDir.get().delete()) {
                LOGGER.warn(Messages.getString("ExplorerURIDropUtil.14") + file.get() + Messages.getString("ExplorerURIDropUtil.15")); //$NON-NLS-1$ //$NON-NLS-2$
            }
            return result;
        } catch (InvocationTargetException | InterruptedException e) {
            LOGGER.warn(Messages.getString("ExplorerURIDropUtil.16") + knimeURI + Messages.getString("ExplorerURIDropUtil.17"), e); //$NON-NLS-1$ //$NON-NLS-2$
            return false;
        }
    }
