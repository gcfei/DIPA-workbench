...
public class GlobalRefreshAction extends ExplorerAction {
    private static final String TOOLTIP = Messages.getString("GlobalRefreshAction.0"); //$NON-NLS-1$
    ...
    public GlobalRefreshAction(final ExplorerView viewer) {
        super(viewer, Messages.getString("GlobalRefreshAction.1")); //$NON-NLS-1$
    ...
    @Override
    public String getId() {
        return "org.knime.workbench.explorer.view.action.refresh"; //$NON-NLS-1$
    }
    ...
    private static class RefreshJob extends ExplorerJob {
        ...
        public RefreshJob(final List<AbstractExplorerFileStore> stores) {
            super(Messages.getString("GlobalRefreshAction.3") + stores.size() + Messages.getString("GlobalRefreshAction.4")); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        protected IStatus run(final IProgressMonitor monitor) {
            monitor.beginTask(Messages.getString("GlobalRefreshAction.5") + m_stores.size() + Messages.getString("GlobalRefreshAction.6"), m_stores.size()); //$NON-NLS-1$ //$NON-NLS-2$

            for (AbstractExplorerFileStore file : m_stores) {
                if (monitor.isCanceled()) {
                    return Status.CANCEL_STATUS;
                }
                monitor.beginTask(Messages.getString("GlobalRefreshAction.7") + file, 1); //$NON-NLS-1$
                ...
                if (file instanceof RemoteExplorerFileStore
                    ...
                    if (workflow != null && workflow instanceof AsyncWorkflowManagerUI) {
                        try {
                            ...
                            return new Status(IStatus.ERROR, "unknown", Messages.getString("GlobalRefreshAction.9"), //$NON-NLS-1$ //$NON-NLS-2$
                                e);
                        }
                    }
                }
