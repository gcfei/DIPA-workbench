...
public class NewWorkflowWizardPage extends WizardPage {
    ...
    private static final String INITIAL_PROJECT_NAME = Messages.getString("NewWorkflowWizardPage.0"); //$NON-NLS-1$

    private static final String DEFAULT_WORKFLOW_GROUP_NAME = Messages.getString("NewWorkflowWizardPage.1"); //$NON-NLS-1$

    private static final String WORKFLOW_GROUP = Messages.getString("NewWorkflowWizardPage.2"); //$NON-NLS-1$

    private static final String WORKFLOW = Messages.getString("NewWorkflowWizardPage.3"); //$NON-NLS-1$

    private static final ImageDescriptor ICON = ExplorerActivator
            .imageDescriptorFromPlugin(ExplorerActivator.PLUGIN_ID,
                    "icons/new_dipa55.png"); //$NON-NLS-1$
    ...
    public NewWorkflowWizardPage(final String[] mountIDs,
        ...
        super("NewWorkflowWizardPage"); //$NON-NLS-1$
        ...
        setTitle(Messages.getString("NewWorkflowWizardPage.6") + m_elementName + Messages.getString("NewWorkflowWizardPage.7")); //$NON-NLS-1$ //$NON-NLS-2$
        setDescription(Messages.getString("NewWorkflowWizardPage.8") + m_elementName.toLowerCase() //$NON-NLS-1$
                + Messages.getString("NewWorkflowWizardPage.9")); //$NON-NLS-1$
    ...
    private void createNameComposite(final Composite parent) {
        ...
        label.setText(Messages.getString("NewWorkflowWizardPage.10") + m_elementName.toLowerCase() //$NON-NLS-1$
                + Messages.getString("NewWorkflowWizardPage.11")); //$NON-NLS-1$
    ...
    private void createDestinationSelectionComposite(final Composite parent) {
        ...
        destinationLabel.setText(Messages.getString("NewWorkflowWizardPage.12") //$NON-NLS-1$
                + m_elementName.toLowerCase() + Messages.getString("NewWorkflowWizardPage.13")); //$NON-NLS-1$
        ...
        browseBtn.setText(Messages.getString("NewWorkflowWizardPage.14")); //$NON-NLS-1$
    ...
    private void handleBrowseButton() {
        ...
        dlg.setTitle(Messages.getString("NewWorkflowWizardPage.15")); //$NON-NLS-1$
        dlg.setHeader(Messages.getString("NewWorkflowWizardPage.16")); //$NON-NLS-1$
        dlg.setDescription(Messages.getString("NewWorkflowWizardPage.17") //$NON-NLS-1$
                + m_parent.getMountID() + Messages.getString("NewWorkflowWizardPage.18")); //$NON-NLS-1$
        dlg.setValidator(new Validator() {
            ...
            public String validateSelectionValue(final AbstractExplorerFileStore selection, final String name) {
                String msg = Messages.getString("NewWorkflowWizardPage.19"); //$NON-NLS-1$
    ...
    private void dialogChanged() {
        ...
        if (projectName.length() == 0) {
            updateStatus(Messages.getString("NewWorkflowWizardPage.20") //$NON-NLS-1$
        ...
        if (newFile.fetchInfo().exists()) {
            updateStatus(Messages.getString("NewWorkflowWizardPage.21")); //$NON-NLS-1$
            return;
        }
        m_projectName = projectName;
        updateStatus(null);
