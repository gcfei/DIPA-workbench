...
public class NewWorkflowWizard extends Wizard implements INewWizard {
    ...
    protected static void throwCoreException(final String message,
        ...
        IStatus status =
                new Status(IStatus.ERROR, "org.knime.workbench.ui", IStatus.OK, //$NON-NLS-1$
                        message, t);
    ...
    public static void createNewWorkflow(final AbstractExplorerFileStore newItem, final IProgressMonitor monitor)
        ...
        if (newItem.fetchInfo().exists()) {
            throwCoreException(Messages.getString("NewWorkflowWizard.2") + newItem.getFullName() //$NON-NLS-1$
                    + Messages.getString("NewWorkflowWizard.3"), null); //$NON-NLS-1$
        }

        if (!newItem.getParent().fetchInfo().exists()) {
            throwCoreException(Messages.getString("NewWorkflowWizard.4") //$NON-NLS-1$
                    + Messages.getString("NewWorkflowWizard.5") //$NON-NLS-1$
                    + Messages.getString("NewWorkflowWizard.6"), null); //$NON-NLS-1$
        }
        ...
        monitor.setTaskName(Messages.getString("NewWorkflowWizard.7")); //$NON-NLS-1$
        ...
        Display.getDefault().asyncExec(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    NodeLogger.getLogger(NewWorkflowWizard.class).info(Messages.getString("NewWorkflowWizard.8") //$NON-NLS-1$
                        + newItem.getFullName() + ": " + ex.getMessage(), ex); //$NON-NLS-1$
                }
            }
        });
    }
