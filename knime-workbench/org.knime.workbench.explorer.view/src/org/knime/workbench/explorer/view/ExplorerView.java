...
public class ExplorerView extends ViewPart implements WorkflowListener,
    ...
    public static final String ID = "org.knime.workbench.explorer.view"; //$NON-NLS-1$
    ...
    public void createPartControl(final Composite parent) {
        ...
        m_copyAction = new CutCopyToClipboardAction(this, "Copy", false); //$NON-NLS-1$
        m_cutAction = new CutCopyToClipboardAction(this, "Move", true); //$NON-NLS-1$
    ...
    private void createLocalToolBar() {
        ...
        exp.setToolTipText(Messages.getString("ExplorerView.3")); //$NON-NLS-1$
        toolBarMgr.add(exp);
        Action coll = new CollapseAction(this);
        coll.setToolTipText(Messages.getString("ExplorerView.4")); //$NON-NLS-1$
        toolBarMgr.add(coll);
        Action collAll = new CollapseAllAction(this);
        collAll.setToolTipText(Messages.getString("ExplorerView.5")); //$NON-NLS-1$
        ...
        if (numOfFlows > 5) {
            MessageBox mb =
                    new MessageBox(getViewSite().getShell(), SWT.ICON_QUESTION
                            | SWT.CANCEL | SWT.OK);
            mb.setText(Messages.getString("ExplorerView.6")); //$NON-NLS-1$
            mb.setMessage(Messages.getString("ExplorerView.7") + numOfFlows + Messages.getString("ExplorerView.8")); //$NON-NLS-1$ //$NON-NLS-2$
            if (mb.open() != SWT.OK) {
                return false;
            }
        }
    ...
    private void hookContextMenu() {
        MenuManager menuMgr = new KNIMEMenuManager("#PopupMenu"); //$NON-NLS-1$
    ...
    private void addGlobalActions(final IMenuManager manager) {
        ...
        boolean showLegacyQuickformAction = Platform.getPreferencesService().getBoolean("org.knime.js.core", //$NON-NLS-1$
            "js.core.enableLegacyQuickformExecution", false, null); //$NON-NLS-1$
        ...
        IMenuManager copyUrlSubmenu = new MenuManager("Copy Location", "copy-url"); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public void saveState(final IMemento memento) {
        final IMemento mementoContent = memento.createChild("content"); //$NON-NLS-1$
        ...
        String expandedMountPoints = Arrays.stream(m_viewer.getExpandedTreePaths()).map(p -> p.getFirstSegment())
                ...
                .map(fs -> fs.getMountID()).collect(Collectors.joining("|")); //$NON-NLS-1$
        mementoContent.putString("expandedElements", expandedMountPoints); //$NON-NLS-1$
    }
    ...
    public void init(final IViewSite site, final IMemento memento)
        ...
        if (memento != null) {
            ...
            content = memento.getChild("content"); //$NON-NLS-1$
            if (content != null) {
                expandedElements = StringUtils.split(content.getString("expandedElements"), '|'); //$NON-NLS-1$
            }
        }
        m_contentDelegator.restoreState(content);
        if (expandedElements == null) {
            // expand LOCAL mount point if this is a new (or very old) workspace
            expandedElements = new String[]{"LOCAL"}; //$NON-NLS-1$
        }
