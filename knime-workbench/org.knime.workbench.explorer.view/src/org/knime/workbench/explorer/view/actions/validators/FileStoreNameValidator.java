...
public class FileStoreNameValidator implements IInputValidator {
    ...
    public String isValid(final String name) {
        if (name == null || name.trim().isEmpty()) {
            return Messages.getString("FileStoreNameValidator.0"); //$NON-NLS-1$
        }
        if (Character.isWhitespace(name.charAt(0))) {
            return Messages.getString("FileStoreNameValidator.1"); //$NON-NLS-1$
        }
        if (Character.isWhitespace(name.charAt(name.length() - 1))) {
            return Messages.getString("FileStoreNameValidator.2"); //$NON-NLS-1$
        }
        if (name.trim().equals(m_initialName)) {
            return Messages.getString("FileStoreNameValidator.3"); //$NON-NLS-1$
        }
