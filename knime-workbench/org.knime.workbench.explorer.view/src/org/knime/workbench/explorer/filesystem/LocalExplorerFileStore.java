            ...
            throws CoreException {
                if (this.equals(destination)) {
                    throw new CoreException(new Status(IStatus.ERROR,
                            ...
                            Messages.LocalExplorerFileStore_0));
                }
                File srcFile = toLocalFile(options, monitor);
                File dstFile = destination.toLocalFile(options, monitor);

                if (dstFile == null) {
                    throw new UnsupportedOperationException(Messages.LocalExplorerFileStore_1
                            + Messages.LocalExplorerFileStore_2
                            + " \"" + destination.getName() + Messages.LocalExplorerFileStore_4); //$NON-NLS-1$
                }
                if (dstFile.exists() && ((options & EFS.OVERWRITE) == 0)) {
                    throw new CoreException(new Status(IStatus.ERROR,
                            ExplorerActivator.PLUGIN_ID,
                            Messages.LocalExplorerFileStore_5
                            + Messages.LocalExplorerFileStore_6));
                }
                ...
                try {
                    ...
                    String message =
                            Messages.LocalExplorerFileStore_7 + srcFile.getAbsolutePath()
                                    + Messages.LocalExplorerFileStore_8 + dstFile.getAbsolutePath() + Messages.LocalExplorerFileStore_9;
                    throw new CoreException(new Status(IStatus.ERROR,
