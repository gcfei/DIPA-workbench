...
public class ExplorerFileSystem extends FileSystem {
     ...
     */
    public static final String SCHEME = "knime"; //$NON-NLS-1$
            ...
            if (relPath.isPresent()) {
                ...
                try {
                    ...
                    NodeLogger.getLogger(getClass())
                        .error(Messages.ExplorerFileSystem_1 + uri + "': " + ex.getMessage(), ex); //$NON-NLS-2$
                }
            }
        }
    ...
    public static String getIDfromURI(final URI uri) {
        if (!SCHEME.equalsIgnoreCase(uri.getScheme())) {
            throw new IllegalArgumentException(Messages.ExplorerFileSystem_3
                    + uri.getScheme() + Messages.ExplorerFileSystem_4 + SCHEME
                    + Messages.ExplorerFileSystem_5);
        }
    ...
    public static String validateFilename(final String name) {
        if (name == null || name.isEmpty()) {
            return Messages.ExplorerFileSystem_6;
        }
        if (name.startsWith(".")) { //$NON-NLS-1$
            return Messages.ExplorerFileSystem_8;
        }
        if (name.endsWith(".")) { //$NON-NLS-1$
            return Messages.ExplorerFileSystem_10;
        }
        Matcher matcher = FileUtil.ILLEGAL_FILENAME_CHARS_PATTERN.matcher(name);
        if (matcher.find()) {
            return Messages.ExplorerFileSystem_11
                    + ExplorerFileSystem.getIllegalFilenameChars() + Messages.ExplorerFileSystem_12;
        }
        return null;
    }
