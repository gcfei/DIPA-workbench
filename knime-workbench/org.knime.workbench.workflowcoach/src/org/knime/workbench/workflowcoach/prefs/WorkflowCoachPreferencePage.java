...
public class WorkflowCoachPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
    ...
    public static final String ID = "org.knime.workbench.workflowcoach"; //$NON-NLS-1$
    ...
    public WorkflowCoachPreferencePage() {
        super(Messages.WorkflowCoachPreferencePage_1);
        setDescription(Messages.WorkflowCoachPreferencePage_2
            + Messages.WorkflowCoachPreferencePage_3
            + Messages.WorkflowCoachPreferencePage_4
            + Messages.WorkflowCoachPreferencePage_5
            + Messages.WorkflowCoachPreferencePage_6
            + Messages.WorkflowCoachPreferencePage_7);
    }
    ...
    protected Control createContents(final Composite parent) {
        Composite composite = createComposite(parent, 1, ""); //$NON-NLS-1$

        /* from community */
        m_checkCommunityProvider = new Button(composite, SWT.CHECK);
        m_checkCommunityProvider.setText(Messages.WorkflowCoachPreferencePage_9);
        ...
        m_checkCommunityProvider.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(final SelectionEvent e) {
                if (getLastUpdate().isPresent() || !m_checkCommunityProvider.getSelection()) {
                    ...
                    setValid(false);
                    setErrorMessage(Messages.WorkflowCoachPreferencePage_10);
                }
        ...
        if (!KNIMECorePlugin.getDefault().getPreferenceStore()
            ...
            hint.setText(
                Messages.WorkflowCoachPreferencePage_11
                    + Messages.WorkflowCoachPreferencePage_12);
        }

        /* update */
        Composite updateComp = createComposite(composite, 3, Messages.WorkflowCoachPreferencePage_13);
        m_noAutoUpdateButton = new Button(updateComp, SWT.RADIO);
        m_noAutoUpdateButton.setText(Messages.WorkflowCoachPreferencePage_14);
        m_weeklyUpdateButton = new Button(updateComp, SWT.RADIO);
        m_weeklyUpdateButton.setText(Messages.WorkflowCoachPreferencePage_15);
        m_monthlyUpdateButton = new Button(updateComp, SWT.RADIO);
        m_monthlyUpdateButton.setText(Messages.WorkflowCoachPreferencePage_16);
        ...
        m_updateButton.setText(Messages.WorkflowCoachPreferencePage_17);
    ...
    private void onUpate() {
        Display.getDefault().syncExec(() -> {
            ...
            m_lastUpdate.setText(Messages.WorkflowCoachPreferencePage_18);
        ...
        UpdateJob.schedule(e -> {
            if (e.isPresent()) {
                Display.getDefault().syncExec(() -> {
                    ...
                    m_lastUpdate.setText(Messages.WorkflowCoachPreferencePage_19);
                    setErrorMessage(Messages.WorkflowCoachPreferencePage_20 + e.get().getMessage()
                        + Messages.WorkflowCoachPreferencePage_21);
                ...
                Display.getDefault().syncExec(() -> {
                    m_updateButton.setEnabled(true);
                    m_lastUpdate.setText(Messages.WorkflowCoachPreferencePage_22);
                    setValid(true);
                    setErrorMessage(null);
                });
            }
    ...
    private void initializeValues() {
        ...
        if (lastUpdate.isPresent()) {
            m_lastUpdate
                .setText(Messages.WorkflowCoachPreferencePage_23 + NodeTripleProvider.LAST_UPDATE_FORMAT.format(lastUpdate.get()));
        } else {
            m_lastUpdate.setText(Messages.WorkflowCoachPreferencePage_24);
        ...
        for (NodeTripleProvider ntp : providers) {
            if (ntp instanceof CommunityTripleProvider) {
                if (m_checkCommunityProvider.getSelection() && ((CommunityTripleProvider)ntp).updateRequired()) {
                    setErrorMessage(Messages.WorkflowCoachPreferencePage_25);
                }
            }
        }
