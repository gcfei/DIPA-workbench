...
public class ExportPreferencesDialog extends Dialog {
    ...
    protected void configureShell(final Shell newShell) {
        ...
        newShell.setText(Messages.ExportPreferencesDialog_0);
    }
    ...
    private void createHeader(final Composite parent) {
        ...
        exec.setText(Messages.ExportPreferencesDialog_1);
        ...
        txt.setText(Messages.ExportPreferencesDialog_2
                + Messages.ExportPreferencesDialog_3);
    ...
    protected void createFileSelection(final Composite parent) {
        ...
        border.setText(Messages.ExportPreferencesDialog_4);
        ...
        browse.setText(Messages.ExportPreferencesDialog_5);
        browse.setToolTipText(Messages.ExportPreferencesDialog_6);
        browse.addSelectionListener(new SelectionListener() {
            @Override
            public void widgetSelected(final SelectionEvent se) {
                FileDialog fileDialog = new FileDialog(getShell(), SWT.SAVE);
                fileDialog.setFilterExtensions(new String[]{"*.epf", "*.*"}); //$NON-NLS-1$ //$NON-NLS-2$
                fileDialog.setText(Messages.ExportPreferencesDialog_9);
                if (m_filename != null) {
    ...
    private void createOverwrite(final Composite parent) {
        ...
        overwriteUI.setText(Messages.ExportPreferencesDialog_12);
        overwriteUI.setToolTipText(Messages.ExportPreferencesDialog_13);
    ...
    private void validate() {
        String errMsg = Messages.ExportPreferencesDialog_14;

        try {
            if (m_filename == null || m_filename.isEmpty()) {
                errMsg = Messages.ExportPreferencesDialog_15;
                return;
            }
            ...
            if (sel.exists()) {
                if (sel.isFile() && sel.canWrite()) {
                    if (!m_overwrite) {
                        errMsg =
                                Messages.ExportPreferencesDialog_16
                                        + Messages.ExportPreferencesDialog_17;
                    } else {
                        errMsg = null;
                    }
                    return;
                ...
                if (sel.isDirectory()) {
                    errMsg = Messages.ExportPreferencesDialog_18;
                    return;
                }
                errMsg = Messages.ExportPreferencesDialog_19;
                return;
            }
