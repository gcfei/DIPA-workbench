...
public class KNIMEEditorPlugin extends AbstractUIPlugin {
    ...
    /** The Plugin ID. */
    public static final String PLUGIN_ID = "org.knime.workbench.editor"; //$NON-NLS-1$
    ...
    public void start(final BundleContext context) throws Exception {
        ...
        final String className = "org.knime.workbench.editor.svgexport.exportservice.WorkflowSVGExportImpl"; //$NON-NLS-1$
        try {
            svgExportClass = Class.forName(className);
        } catch (ClassNotFoundException cnfe) {
            NodeLogger.getLogger(getClass()).debug(Messages.KNIMEEditorPlugin_2
                + className + "\""); //$NON-NLS-1$
        }
        if (svgExportClass != null) {
            try {
                ...
                NodeLogger.getLogger(getClass()).error(
                    Messages.KNIMEEditorPlugin_4 + WorkflowSVGExport.class.getName() + Messages.KNIMEEditorPlugin_5, e);
            }
        }
