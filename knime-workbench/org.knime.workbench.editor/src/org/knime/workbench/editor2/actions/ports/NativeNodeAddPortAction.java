...
/**
 * An port action for native node container nodes that allows to add ports.
 *
 * @author Mark Ortmann, KNIME GmbH, Berlin, Germany
 */
final class NativeNodeAddPortAction extends AbstractNativeNodePortAction {
     ...
     */
    NativeNodeAddPortAction(final NodeContainerEditPart editPart,
    ...
    protected boolean modifyPorts() {
        ExtendablePortGroup extPortGrp = getPortGroup(ExtendablePortGroup.class);
        final PortType[] supportedTypes = extPortGrp.getSupportedPortTypes();
        final PortType toAdd;
        if (supportedTypes.length > 1) {
            toAdd = createDialog(Messages.NativeNodeAddPortAction_0 + m_grpName + Messages.NativeNodeAddPortAction_1, supportedTypes);
        } else {
            toAdd = supportedTypes[0];
        }
        if (toAdd != null) {
            extPortGrp.addPort(toAdd);
            return true;
        }
        return false;
    }
