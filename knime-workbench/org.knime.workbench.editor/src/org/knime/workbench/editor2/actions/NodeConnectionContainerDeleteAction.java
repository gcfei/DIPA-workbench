...
public class NodeConnectionContainerDeleteAction extends DeleteAction {
    ...
    public Command createDeleteCommand(final List objects) {
        ...
        StringBuilder dialogText = new StringBuilder("您真的要删除 ");
        if (nodeCount > 0) {
            dialogText.append(nodeCount).append(" 个节点");
            dialogText.append(nodeCount > 1 ? "" : "");;
        ...
        if (connCount > 0) {
            dialogText.append(connCount).append(" 个连接");
            dialogText.append(connCount > 1 ? "" : "");
            dialogText.append(annoCount > 0 ? " 和 " : "");
        }
        if (annoCount > 0) {
            dialogText.append(annoCount).append(" 个注释");
            dialogText.append(annoCount > 1 ? "" : "");
        }
        dialogText.append("?");
