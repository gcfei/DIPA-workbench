...
public class LockMetaNodeDialog extends IconAndMessageDialog {
    ...
    public LockMetaNodeDialog(final Shell parent, final WorkflowManager mgr) {
        super(parent);
        super.message = Messages.LockMetaNodeDialog_0; //$NON-NLS-1$
        m_workflow = mgr;
    ...
    protected Control createDialogArea(final Composite parent) {
        ...
        m_enableChecker.setText(Messages.LockMetaNodeDialog_1); //$NON-NLS-1$
        ...
        m_showPlainChecker.setText(Messages.LockMetaNodeDialog_5); //$NON-NLS-1$
    ...
    private void updateEnableStatus() {
        ...
        if (!isEnabled) {
            m_passwordTextField.setText(""); //$NON-NLS-1$
        } else {
    ...
    protected void configureShell(final Shell newShell) {
        ...
        newShell.setText(Messages.LockMetaNodeDialog_7); //$NON-NLS-1$
    }
    ...
    public void create() {
        ...
        if (os != null && os.toLowerCase().startsWith("linux")) { //$NON-NLS-1$
    ...
    protected void okPressed() {
        ...
        if (enabled && (pass == null || pass.isEmpty())) {
            MessageDialog.openError(getParentShell(), Messages.LockMetaNodeDialog_10, //$NON-NLS-1$
                    Messages.LockMetaNodeDialog_11); //$NON-NLS-1$
        } else {
            if (enabled) {
                try {
                    if (!verifyPassword(pass, m_passwordKnimeCOMTextField.getText())) {
                        MessageDialog.openError(getParentShell(),
                            Messages.LockMetaNodeDialog_12, Messages.LockMetaNodeDialog_13 //$NON-NLS-1$ //$NON-NLS-2$
                                                + Messages.LockMetaNodeDialog_14 //$NON-NLS-1$
                                                + Messages.LockMetaNodeDialog_15); //$NON-NLS-1$
                    } else {
                        m_newPassword = pass;
                    ...
                    MessageDialog.openError(getParentShell(),
                            Messages.LockMetaNodeDialog_16, Messages.LockMetaNodeDialog_17 //$NON-NLS-1$ //$NON-NLS-2$
                            + Messages.LockMetaNodeDialog_18); //$NON-NLS-1$
                    return;
                }
            } else {
                m_newPassword = null;
    ...
    private static boolean verifyPassword(final String password, final String encryptedPassword)
        ...
        BufferedReader reader = new BufferedReader(new InputStreamReader(decr, Charset.forName("UTF-8"))); //$NON-NLS-1$
