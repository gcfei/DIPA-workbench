...
public class ConfigureMetaNodePortsPage extends WizardPage {
    ...
    private static final String DESCRIPTION = Messages.ConfigureMetaNodePortsPage_0
        + Messages.ConfigureMetaNodePortsPage_1;
    ...
    public void setVisible(final boolean visible) {
        ...
        if (visible) {
            ...
            if (m_metaNode != null) {
                ...
                m_name.setText(Messages.ConfigureMetaNodePortsPage_2);
            }
    ...
    private void populateInfoListsFromTemplate() {
        LOGGER.debug(Messages.ConfigureMetaNodePortsPage_3 + m_template);
        // clear all fields
        m_inPortList.clear();
        m_outPortList.clear();
        if (m_template == null) {
    ...
    private void populateSelectionListsFromInfoList() {
        populateSelectionlistFromInfolist(m_inPorts, m_inPortList, "in_"); //$NON-NLS-1$
        populateSelectionlistFromInfolist(m_outPorts, m_outPortList, "out_"); //$NON-NLS-1$
    }
    ...
    private void populateSelectionlistFromInfolist(final List selList, final java.util.List<MetaPortInfo> infoList,
        ...
        for (int i = m_offset; i < infoList.size(); i++) {
            ...
            String listEntry = namePrefix + i + " (" + info.getType().getName() + ")"; //$NON-NLS-1$ //$NON-NLS-2$
    ...
    private void updateStatus() {
        ...
        if (idx >= 0) {
            ...
            str.append(inPort ? "Input " : "Output "); //$NON-NLS-1$ //$NON-NLS-2$
            str.append("port "); //$NON-NLS-1$
            str.append(idx);
            ...
            if (info.getOldIndex() >= 0) {
                if (info.isConnected()) {
                    if (info.getMessage() != null && !info.getMessage().isEmpty()) {
                        ...
                        str.append("Connected. "); //$NON-NLS-1$
                    }
                } else {
                    str.append("Not connected. "); //$NON-NLS-1$
                }
                if (info.getOldIndex() != idx + m_offset) {
                    str.append("Moved from old index "); //$NON-NLS-1$
                    str.append(info.getOldIndex());
                    str.append(". "); //$NON-NLS-1$
                }
            } else {
                str.append("New port. "); //$NON-NLS-1$
            }
    ...
    private void createPortPart(final Composite parent, final boolean in) {
        ...
        if (in) {
            ...
            label.setText(Messages.ConfigureMetaNodePortsPage_20);
            ...
            m_addInPort.setText(Messages.ConfigureMetaNodePortsPage_21);
            ...
            m_remInPort.setText(Messages.ConfigureMetaNodePortsPage_22);
            ...
            m_upInPort.setText(Messages.ConfigureMetaNodePortsPage_23);
            ...
            m_downInPort.setText(Messages.ConfigureMetaNodePortsPage_24);
            ...
            Label label = new Label(left, SWT.NONE);
            label.setText(Messages.ConfigureMetaNodePortsPage_25);
            ...
            m_addOutPort.setText(Messages.ConfigureMetaNodePortsPage_26);
            ...
            m_remOutPort.setText(Messages.ConfigureMetaNodePortsPage_27);
            ...
            m_upOutPort.setText(Messages.ConfigureMetaNodePortsPage_28);
            ...
            m_downOutPort.setText(Messages.ConfigureMetaNodePortsPage_29);
    ...
    private void addPort(final boolean inPort) {
        ...
        if (port != null) {
            ...
            infoList.add(info);
            populateSelectionlistFromInfolist(portList, infoList, inPort ? "in_" : "out_"); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    private void removeSelPort(final boolean inPort) {
        ...
        if (idx >= infoList.size()) {
            LOGGER.coding(Messages.ConfigureMetaNodePortsPage_32);
            return;
        }
    ...
    private void moveSelPort(final boolean inPort, final boolean moveUp) {
        ...
        if (idx > maxIdx) {
            LOGGER.coding(Messages.ConfigureMetaNodePortsPage_35
                    + (moveUp ? Messages.ConfigureMetaNodePortsPage_36 : Messages.ConfigureMetaNodePortsPage_37));
            return;
        }
    ...
    private void createTopPart(final Composite parent) {
        ...
        if (m_metaNode != null) {
            label.setText(Messages.ConfigureMetaNodePortsPage_40);
        } else {
            label.setText(Messages.ConfigureMetaNodePortsPage_41);
        }
        ...
        m_previewPanel.addPaintListener(new PaintListener() {
            ...
            public void paintControl(final PaintEvent e) {
                ...
                gc.drawImage(ImageRepository.getImage(KNIMEEditorPlugin.PLUGIN_ID,
                    "/icons/meta/meta_custom_preview.png"), //$NON-NLS-1$
