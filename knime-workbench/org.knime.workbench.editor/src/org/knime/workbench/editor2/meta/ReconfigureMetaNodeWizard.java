...
public class ReconfigureMetaNodeWizard extends Wizard {
    ...
    public void addPages() {
        String name = m_metaNode != null ? Messages.ReconfigureMetaNodeWizard_0 : Messages.ReconfigureMetaNodeWizard_1;
        setWindowTitle(Messages.ReconfigureMetaNodeWizard_2 + name + Messages.ReconfigureMetaNodeWizard_3);
        ...
        m_addPage = new ConfigureMetaNodePortsPage(Messages.ReconfigureMetaNodeWizard_5 + name + Messages.ReconfigureMetaNodeWizard_6);
        if (m_metaNode != null) {
    ...
    public boolean performFinish() {
        ...
        if (inPortChanges) {
            infoStr.append(Messages.ReconfigureMetaNodeWizard_7);
        }
        if (outPortChanges) {
            infoStr.append(Messages.ReconfigureMetaNodeWizard_8);
        }
        if (nameChange) {
            infoStr.append(Messages.ReconfigureMetaNodeWizard_9);
        }
        if (infoStr.length() == 0) {
            LOGGER.info(Messages.ReconfigureMetaNodeWizard_10);
            return true;
        }
        infoStr.insert(0, Messages.ReconfigureMetaNodeWizard_11);
        infoStr.append(Messages.ReconfigureMetaNodeWizard_12 + node.getID());
        LOGGER.info(infoStr);
