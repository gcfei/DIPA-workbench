...
public class StyledTextEditor extends CellEditor implements WorkflowCanvasClickListener.AnnotationModeExitListener {
    ...
    private static final RGB[] DEFAULT_COLORS = new RGB[]{
        ColorUtilities.fromHex("CDE280"), ColorUtilities.fromHex("D8D37B"), //$NON-NLS-1$ //$NON-NLS-2$
        ColorUtilities.fromHex("93DDD2"), ColorUtilities.fromHex("D0D2B5"), //$NON-NLS-1$ //$NON-NLS-2$
        ColorUtilities.fromHex("ADDF9E"), ColorUtilities.fromHex("E8AFA7"), //$NON-NLS-1$ //$NON-NLS-2$
        ColorUtilities.fromHex("C4CBE0"), ColorUtilities.fromHex("E3B67D")}; //$NON-NLS-1$ //$NON-NLS-2$
    ...
    private static void disableHandlerWithItalicKeybindingConflict () {
        if (! DISABLED_MOD1_I_HANDLER.getAndSet(true)) {
            ...
            Command command = cService.getCommand("org.eclipse.ui.file.properties"); //$NON-NLS-1$

            command.undefine();

            LOGGER.debug(Messages.StyledTextEditor_9);
        }
    }
    ...
    private Control createStyledText(final Composite parent) {
        ...
        m_styledText.setText(""); //$NON-NLS-1$
    ...
    protected Object doGetValue() {
        assert m_styledText != null : Messages.StyledTextEditor_11;
    ...
    protected void doSetFocus() {
        assert m_styledText != null : Messages.StyledTextEditor_12;
    ...
    protected void doSetValue(final Object value) {
        assert value instanceof Annotation : Messages.StyledTextEditor_13;
    ...
    private List<StyleRange> getStylesInSelection() {
        ...
        if ((styles == null) || (styles.length == 0)) {
            ...
            for (final StyleRange s : styles) {
                if (s.start < lastEnd) {
                    LOGGER.error(Messages.StyledTextEditor_14);
                }
                if (lastEnd < s.start) {
    ...
    private void addMenu(final Composite parent) {
        ...
        // background color
        image = ImageRepository.getImage(KNIMEEditorPlugin.PLUGIN_ID, "icons/annotations/bgcolor_10.png"); //$NON-NLS-1$
        action = addMenuItem(menu, "bg", SWT.PUSH, Messages.StyledTextEditor_17, image); //$NON-NLS-1$

        // alignment
        image = ImageRepository.getImage(KNIMEEditorPlugin.PLUGIN_ID, "icons/annotations/alignment_10.png"); //$NON-NLS-1$

        MenuItem alignmentMenuItem = addMenuItem(menu, Messages.StyledTextEditor_19, SWT.CASCADE, Messages.StyledTextEditor_20, image);

        final Menu alignMenu = new Menu(alignmentMenuItem);
        alignmentMenuItem.setMenu(alignMenu);

        m_leftAlignMenuItem = addMenuItem(alignMenu, Messages.StyledTextEditor_21, SWT.RADIO, Messages.StyledTextEditor_22, null);
        m_leftAlignMenuItem.setSelection(true);

        m_centerAlignMenuItem = addMenuItem(alignMenu, Messages.StyledTextEditor_23, SWT.RADIO, Messages.StyledTextEditor_24, null);

        m_rightAlignMenuItem = addMenuItem(alignMenu, Messages.StyledTextEditor_25, SWT.RADIO, Messages.StyledTextEditor_26, null);
        ...
        // font/style button
        image = ImageRepository.getImage(KNIMEEditorPlugin.PLUGIN_ID, "icons/annotations/font_10.png"); //$NON-NLS-1$
        action = addMenuItem(menu, Messages.StyledTextEditor_28, SWT.PUSH, Messages.StyledTextEditor_29, image);
        m_enableOnSelectedTextMenuItems.add(action);

        new MenuItem(menu, SWT.SEPARATOR);

        // border style
        image = ImageRepository.getImage(KNIMEEditorPlugin.PLUGIN_ID, "icons/annotations/border_10.png"); //$NON-NLS-1$
        m_borderMenuItem = addMenuItem(menu, Messages.StyledTextEditor_31, SWT.PUSH, Messages.StyledTextEditor_32, image);
        m_postBorderMenuItemSeparator = new MenuItem(menu, SWT.SEPARATOR);

        // ok button
        image = ImageRepository.getImage(KNIMEEditorPlugin.PLUGIN_ID, "icons/annotations/ok_10.png"); //$NON-NLS-1$
        addMenuItem(menu, Messages.StyledTextEditor_34, SWT.PUSH, Messages.StyledTextEditor_35, image);

        // cancel button
        image = ImageRepository.getImage(KNIMEEditorPlugin.PLUGIN_ID, "icons/annotations/cancel_10.png"); //$NON-NLS-1$
        addMenuItem(menu, Messages.StyledTextEditor_37, SWT.PUSH, Messages.StyledTextEditor_38, image);
    ...
    private void buttonClick(final String src) {
        if (src.equals("style")) { //$NON-NLS-1$
            font();
            fireEditorValueChanged(true, true);
        } else if (src.equals("color")) { //$NON-NLS-1$
            fontColor();
            fireEditorValueChanged(true, true);
        } else if (src.equals("bold")) { //$NON-NLS-1$
            bold();
        } else if (src.equals("italic")) { //$NON-NLS-1$
            italic();
        } else if (src.equals("bg")) { //$NON-NLS-1$
            bgColor();
            fireEditorValueChanged(true, true);
        } else if (src.equals("alignment_left")) { //$NON-NLS-1$
            alignment(SWT.LEFT);
            m_toolbar.updateToolbarToReflectState();
        } else if (src.equals("alignment_center")) { //$NON-NLS-1$
            alignment(SWT.CENTER);
            m_toolbar.updateToolbarToReflectState();
        } else if (src.equals("alignment_right")) { //$NON-NLS-1$
            alignment(SWT.RIGHT);
            m_toolbar.updateToolbarToReflectState();
        } else if (src.equals("border")) { //$NON-NLS-1$
            borderStyle();
            fireEditorValueChanged(true, true);
        } else if (src.equals("ok")) { //$NON-NLS-1$
            ok();
        } else if (src.equals("cancel")) { //$NON-NLS-1$
            cancel();
        } else {
            LOGGER.coding(Messages.StyledTextEditor_50);
        }

        // set the focus back to the editor after the buttons finish
        if (!src.equals("ok") && !src.equals("cancel")) { //$NON-NLS-1$ //$NON-NLS-2$
            m_styledText.setFocus();
        }
    }
    ...
    private void checkSelectionOfAlignmentMenuItems(final int swtAlignment) {
        ...
        switch (swtAlignment) {
            ...
            case SWT.RIGHT:
                ...
                break;
            default:
                LOGGER.coding(Messages.StyledTextEditor_53 + swtAlignment);
                return;
        }
    ...
    private void fontColor() {
        ...
        colorDialog.setText(Messages.StyledTextEditor_54);
    ...
    private void bgColor() {
        ...
        colorDialog.setText(Messages.StyledTextEditor_55);
