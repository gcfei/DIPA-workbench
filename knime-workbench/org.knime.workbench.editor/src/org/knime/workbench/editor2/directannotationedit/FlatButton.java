...
/**
 * This class provides a flat button with potentially an image and/or a post-renderer.
 *
 ...
 *
 * <b>Note:</b> This class does not current support the notion of focus.
 *
...
public class FlatButton extends Canvas {
    ...
    private static synchronized void initializeColorsIfNecessary() {
        if (MOUSE_OVER_OVERLAY_COLOR == null) {
            ...
            MOUSE_OVER_OVERLAY_COLOR = ColorDropDown.createColorFromHexString(d, "#D8E6F2"); //$NON-NLS-1$
            MOUSE_OVER_BORDER_COLOR = ColorDropDown.createColorFromHexString(d, "#C0DCF3"); //$NON-NLS-1$
            MOUSE_DOWN_OVERLAY_COLOR = MOUSE_OVER_BORDER_COLOR;
            SELECTED_BACKGROUND_COLOR = MOUSE_DOWN_OVERLAY_COLOR;
            SELECTED_BORDER_COLOR = ColorDropDown.createColorFromHexString(d, "#90C8F6"); //$NON-NLS-1$
        }
    }
    ...
    protected FlatButton(final Composite parent, final int style, final Image buttonImage, final Image altImage,
        ...
        if (style == SWT.RADIO) {
            throw new IllegalArgumentException(Messages.FlatButton_3);
        }

        if ((buttonImage == null) && !nullImageAcceptable) {
            throw new IllegalArgumentException(Messages.FlatButton_4);
        }

        m_buttonIsStateful = (style == SWT.TOGGLE);
