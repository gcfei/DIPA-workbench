...
public final class InstallMissingNodesJob extends Job {
    ...
    InstallMissingNodesJob(final List<NodeAndBundleInformationPersistor> missingNodes,
        ...
        super("Find missing extensions"); //$NON-NLS-1$
    ...
    protected IStatus run(final IProgressMonitor monitor) {
        ...
        if (!status.isOK()) {
            ...
            Display.getDefault().asyncExec(() -> {
                MessageDialog.openWarning(SWTUtilities.getActiveShell(), Messages.InstallMissingNodesJob_2,
                    Messages.InstallMissingNodesJob_3);
            });
            return Status.OK_STATUS;
        } else {
            if (!m_missingComponents.isEmpty()) {
                Display.getDefault().syncExec(() -> {
                    MessageDialog.openWarning(SWTUtilities.getActiveShell(), Messages.InstallMissingNodesJob_4,
                        Messages.InstallMissingNodesJob_5 + m_missingComponents.stream()
                            .map(i -> i.getComponentName()).collect(Collectors.joining(", "))); //$NON-NLS-1$
                });
            }
                ...
                if (PlatformUI.isWorkbenchRunning() && event.getResult().isOK()) {
                    Display.getDefault().asyncExec(() -> {
                        if (Display.getDefault().isDisposed()) {
                            NodeLogger.getLogger(Messages.InstallMissingNodesJob_7);
    ...
    private IStatus findExtensions(final IProgressMonitor monitor,
        ...
        try {
            ...
            NodeLogger.getLogger(getClass()).error(Messages.InstallMissingNodesJob_8 + ex.getMessage(), ex);
            return new Status(IStatus.ERROR, myself.getSymbolicName(),
                Messages.InstallMissingNodesJob_9, ex);
        }
    }
    ...
    private static boolean searchInRepository(final IMetadataRepository repository,
        ...
        if (componentInfo.getFeatureSymbolicName().isPresent()) {
            ...
            IQuery<IInstallableUnit> featureQuery = QueryUtil.createLatestQuery(QueryUtil.createQuery(
                "everything.select(y | y.properties ~= filter(\"(org.eclipse.equinox.p2.type.group=true)\") " //$NON-NLS-1$
                    + "&& everything.select(x | x.id == $0).exists(r | y.requirements.exists(v | r ~= v)))", //$NON-NLS-1$
