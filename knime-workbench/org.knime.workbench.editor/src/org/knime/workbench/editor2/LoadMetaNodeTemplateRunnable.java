...
public class LoadMetaNodeTemplateRunnable extends PersistWorkflowRunnable {
    ...
    public void run(final IProgressMonitor pm) {
        try {
            ...
            ProgressHandler progressHandler = new ProgressHandler(pm, 101, "Loading instance..."); //$NON-NLS-1$
            ...
            if (parentFile.isFile()) {
                //unzip
                File tempDir = FileUtil.createTempDir("template-workflow"); //$NON-NLS-1$
                FileUtil.unzip(parentFile, tempDir);
                Files.delete(parentFile.toPath());
                File[] extractedFiles = tempDir.listFiles();
                if (extractedFiles.length == 0) {
                    throw new IOException(Messages.LoadMetaNodeTemplateRunnable_2 + parentFile + Messages.LoadMetaNodeTemplateRunnable_3);
                }
            ...
            MetaNodeLinkUpdateResult loadResult =
                    new MetaNodeLinkUpdateResult(Messages.LoadMetaNodeTemplateRunnable_4 + m_templateURI + "\""); //$NON-NLS-2$
            ...
            switch (status.getSeverity()) {
                case IStatus.OK:
                    message = Messages.LoadMetaNodeTemplateRunnable_7;
                    break;
                case IStatus.WARNING:
                    message = Messages.LoadMetaNodeTemplateRunnable_8;
                    break;
                default:
                    message = Messages.LoadMetaNodeTemplateRunnable_9;
            }
