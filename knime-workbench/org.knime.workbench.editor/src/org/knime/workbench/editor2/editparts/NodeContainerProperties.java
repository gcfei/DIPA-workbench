...
public class NodeContainerProperties implements IPropertySource {
    ...
    private static final String PROP_INVALID_ID =
            "knime.workbench.settingsview.invalid"; //$NON-NLS-1$
    ...
    public NodeContainerProperties(final NodeContainer node) {
        ...
        NodeSettings nodeSettings = new NodeSettings("Props"); //$NON-NLS-1$
        WorkflowManager wfm = node.getParent();
        try {
            wfm.saveNodeSettings(node.getID(), nodeSettings);
            m_settings = nodeSettings.getConfig("model"); //$NON-NLS-1$
        } catch (InvalidSettingsException e) {
            m_settings = new NodeSettings("<empty>"); //$NON-NLS-1$
        }
        m_prefix = ""; //$NON-NLS-1$
    }
    ...
    public IPropertyDescriptor[] getPropertyDescriptors() {
        ...
        if (m_settings != null) {
            ...
            return new IPropertyDescriptor[]{new PropertyDescriptor(
                    PROP_INVALID_ID,
                    Messages.NodeContainerProperties_6)};
        }
    ...
    protected IPropertyDescriptor[] getDescriptors() {
        ...
        for (Enumeration<TreeNode> it = m_settings.children(); it
            ...
            if (prop instanceof Config) {
                ...
                String label = prop.getKey() + " (" + typeName + ")"; //$NON-NLS-1$ //$NON-NLS-2$
                switch (prop.getType()) {
    ...
    public Object getPropertyValue(final Object id) {
        if (id instanceof String) {
            ...
            if (!hierID.startsWith(m_prefix)) {
                return Messages.NodeContainerProperties_9 + hierID
                        + Messages.NodeContainerProperties_10 + m_prefix + ")"; //$NON-NLS-2$
            }
            String ourID = hierID;
            if (!m_prefix.isEmpty()) {
            ...
            if (entry instanceof Config) {
                ...
                if (entry == null) {
                    return Messages.NodeContainerProperties_12 + ourID;
                }
    ...
    public void resetPropertyValue(final Object id) {
        LOGGER.debug("Reset Property Value. id=" + id); //$NON-NLS-1$
    }
    ...
    public void setPropertyValue(final Object id, final Object value) {
        if ((id instanceof String) && (value instanceof String)) {
            ...
            if (strID.startsWith(m_prefix)) {
                ...
                WorkflowManager wfm = node.getParent();
                NodeSettings nodeSettings = new NodeSettings("Transfer"); //$NON-NLS-1$
                NodeSettings settings;
                try {
                    wfm.saveNodeSettings(node.getID(), nodeSettings);
                    // overwrite our config in the settings
                    settings = nodeSettings.getNodeSettings("model"); //$NON-NLS-1$
                ...
                try {
                    ...
                    LOGGER.error(Messages.NodeContainerProperties_16 + strVal + "): " //$NON-NLS-2$
                            + ex.getMessage(), ex);
                    return;
                }
