...
public class NewWorkflowContainerEditPolicy extends ContainerEditPolicy {
     ...
     */
    private static final String FEATURE_GROUP_SUFFIX = ".feature.group"; //$NON-NLS-1$
     ...
     */
    public static final String REQ_LINK_METANODE_TEMPLATE = Messages.NewWorkflowContainerEditPolicy_1;

    /**
     * 2018.12.31 - i can find no usage of this String in a search across BitBucket
     */
    public static final String REQ_COPY_METANODE_TEMPLATE = Messages.NewWorkflowContainerEditPolicy_2;
    ...
    protected Command getCreateCommand(final CreateRequest request) {
        ...
        if (request instanceof CreateDropRequest) {
            ...
            if (obj instanceof AbstractExplorerFileStore) {
                ...
                LOGGER.error(Messages.NewWorkflowContainerEditPolicy_3 + obj);
            }
        }
        return null;
    }
    ...
    private Command handleURLDrop(final WorkflowManagerUI managerUI, final Optional<WorkflowManager> manager, final URL url,
        ...
        if ((uri = createEncodedURI(url).orElse(null)) != null) {
            ...
            showPopup(Messages.NewWorkflowContainerEditPolicy_4, Messages.NewWorkflowContainerEditPolicy_5 + uri.getPath() + Messages.NewWorkflowContainerEditPolicy_6,
                SWT.ICON_WARNING);
            LOGGER.debug(Messages.NewWorkflowContainerEditPolicy_7 + uri);
        }
        return null;
    }
    ...
    private static void handleExtensionDrop(final String featureName, final String featureSymbolicName) {
        if (isFeatureInstalled(featureSymbolicName)) {
            showPopup(Messages.NewWorkflowContainerEditPolicy_8, Messages.NewWorkflowContainerEditPolicy_9 + featureName + Messages.NewWorkflowContainerEditPolicy_10,
                SWT.ICON_INFORMATION);
            return;
        } else {
            final String[] dialogButtonLabels = {IDialogConstants.YES_LABEL, IDialogConstants.NO_LABEL};
            MessageDialog dialog = new MessageDialog(SWTUtilities.getActiveShell(), Messages.NewWorkflowContainerEditPolicy_11, null,
                Messages.NewWorkflowContainerEditPolicy_12 + featureName + "'?", MessageDialog.QUESTION, //$NON-NLS-2$
    ...
    private Command handleNodeDropFromURI(final WorkflowManagerUI manager, final NodeImport nodeImport,
        ...
        if (nodeTemplate != null) {
            try {
                ...
                //shouldn't happen
                LOGGER.error(Messages.NewWorkflowContainerEditPolicy_14 + nodeFactory + Messages.NewWorkflowContainerEditPolicy_15 + nodeName + ")", e); //$NON-NLS-3$
                return null;
            }
        } else {
            ...
            MessageDialog dialog = new MessageDialog(shell, Messages.NewWorkflowContainerEditPolicy_17, null,
                Messages.NewWorkflowContainerEditPolicy_18 + featureName + Messages.NewWorkflowContainerEditPolicy_19
                    + Messages.NewWorkflowContainerEditPolicy_20,
    ...
    private static NodeTemplate getNodeTemplate(final String factoryName, final String nodeName,
        ...
        if (isDynamicNode) {
            templateId = factoryName + "#" + nodeName; //$NON-NLS-1$
        }
        ...
        if (template == null && !isDynamicNode) {
            templateId = templateId + "#" + nodeName; //$NON-NLS-1$
