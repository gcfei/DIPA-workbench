...
public class FontStore {
    ...
    private static Font defaultFont() {
        ...
        Font defFont = null;
        String name = Messages.FontStore_0;
        try {
            if (installedFontFamilyNames.contains(name)) {
                defFont = new Font(Display.getDefault(), name, defFontSize, SWT.NORMAL);
            }
        } catch (SWTError e) {
            LOGGER.warn(Messages.FontStore_1 + name + Messages.FontStore_2
                    + Messages.FontStore_3, e);
        }
        if (defFont == null) {
            // Fall back to "common" fonts similar to Arial
            if (Platform.OS_MACOSX.equals(Platform.getOS())) {
                defFont = tryLoadFallbackFont(defFontSize, "Trebuchet MS", installedFontFamilyNames); //$NON-NLS-1$
            } else if (Platform.OS_LINUX.equals(Platform.getOS())) {
                defFont = tryLoadFallbackFont(defFontSize, "Nimbus Sans L", installedFontFamilyNames); //$NON-NLS-1$
            } else {
                defFont = tryLoadFallbackFont(defFontSize, "Microsoft Sans Serif", installedFontFamilyNames); //$NON-NLS-1$
                if (defFont == null) {
                    defFont = tryLoadFallbackFont(defFontSize, "Trebuchet MS", installedFontFamilyNames); //$NON-NLS-1$
                }
            }
        ...
        if (defFont == null) {
            ...
            LOGGER.warn(Messages.FontStore_8 + defFont);
        }
        return defFont;
    }
    ...
    private static Font tryLoadFallbackFont(final int defFontSize, final String name,
        ...
        if (installedFontFamilyNames.contains(name)) {
            try {
                ...
                LOGGER.warn(Messages.FontStore_9 + name + Messages.FontStore_10, e);
            }
        }
        return null;
    }
