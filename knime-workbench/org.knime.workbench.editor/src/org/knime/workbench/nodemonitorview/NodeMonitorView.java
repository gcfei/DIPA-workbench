...
public class NodeMonitorView extends ViewPart implements ISelectionListener, LocationListener, NodeStateChangeListener {
    ...
    public void createPartControl(final Composite parent) {
        ...
        final RetargetAction pinButton =
            new RetargetAction("PinView", Messages.NodeMonitorView_1, IAction.AS_CHECK_BOX); //$NON-NLS-1$
        pinButton.setImageDescriptor(ImageDescriptor.createFromFile(this.getClass(), "icons/pin.png")); //$NON-NLS-1$
        ...
        final RetargetAction menuentrytable =
            new RetargetAction("OutputTable", Messages.NodeMonitorView_4, IAction.AS_RADIO_BUTTON); //$NON-NLS-1$
        ...
        final RetargetAction dropdownmenuvars =
            new RetargetAction("NodeVariables", Messages.NodeMonitorView_6, IAction.AS_RADIO_BUTTON); //$NON-NLS-1$
        ...
        final RetargetAction menuentrysettings =
            new RetargetAction("NodeConf", Messages.NodeMonitorView_8, IAction.AS_RADIO_BUTTON); //$NON-NLS-1$
        ...
        final RetargetAction menuentryallsettings =
            new RetargetAction("NodeConfAll", Messages.NodeMonitorView_10, IAction.AS_RADIO_BUTTON); //$NON-NLS-1$
        ...
        final RetargetAction menuentrynodetimer =
            new RetargetAction("NodeTimer", Messages.NodeMonitorView_12, IAction.AS_RADIO_BUTTON); //$NON-NLS-1$
        ...
        final RetargetAction menuentrygraphannotations =
            new RetargetAction("NodeGraphAnno", Messages.NodeMonitorView_14, IAction.AS_RADIO_BUTTON); //$NON-NLS-1$
        ...
        titlelabel.setText(Messages.NodeMonitorView_15);
        ...
        m_info.setText("n/a.                        "); //$NON-NLS-1$
        m_portIndex = new ComboViewer(infoPanel);
        m_portIndex.add(new String[]{"port 0", "port 1", "port 2"}); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        ...
        m_loadButton.setText(Messages.NodeMonitorView_23);
        m_loadButton.setToolTipText(Messages.NodeMonitorView_24);
        ...
        String[] titles = {Messages.NodeMonitorView_25, Messages.NodeMonitorView_26};
    ...
    public void selectionChanged(final IWorkbenchPart part, final ISelection selection) {
        ...
        if (structSel.size() < 1) {
            // Nothing selected
            m_title.setText(""); //$NON-NLS-1$
            m_state.setText(Messages.NodeMonitorView_28);
            m_table.removeAll();
            return;
        }
        if (structSel.size() > 1) {
            ...
            m_title.setText(""); //$NON-NLS-1$
            m_state.setText(Messages.NodeMonitorView_30);
            m_table.removeAll();
            return;
        }
        ...
        if (sel instanceof NodeContainerEditPart) {
            ...
            warningMessage(Messages.NodeMonitorView_31);
        }
    }
    ...
    private void updateNodeContainerInfo(final NodeContainerUI nc) {
        ...
        m_title.setText(nc.getName() + "  (" + nc.getID() + ")"); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        for (int i = 0; i < nrPorts; i++) {
            vals[i] = Messages.NodeMonitorView_35 + i;
        }
        ...
        switch (m_choice) {
            ...
            case GRAPHANNOTATIONS:
                ...
                break;
            default:
                throw new AssertionError(Messages.NodeMonitorView_36 + m_choice);
        }
    ...
    private void resetMonitorTable() {
        m_loadButton.setEnabled(false);
        m_loadButton.setText(Messages.NodeMonitorView_37);
        ...
        public void widgetSelected(final SelectionEvent e) {
            try {
                ...
                PlatformUI.getWorkbench().getProgressService().busyCursorWhile((monitor) -> {
                    monitor.beginTask(Messages.NodeMonitorView_38, 100);
                    try {
