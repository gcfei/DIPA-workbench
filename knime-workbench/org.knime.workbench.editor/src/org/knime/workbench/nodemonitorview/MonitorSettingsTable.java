...
public class MonitorSettingsTable implements NodeMonitorTable {
    ...
    public void setupTable(final Table table) {
       ...
       String[] titles = {Messages.MonitorSettingsTable_0, Messages.MonitorSettingsTable_1};
        ...
        if (m_nodeAndBundleInfo != null && m_showAll) {
            ...
            item4.setText(0, Messages.MonitorSettingsTable_2);
            item4.setText(1, m_nodeAndBundleInfo.getFeatureName().orElse("?")); //$NON-NLS-1$

            TableItem item5 = new TableItem(table, SWT.NONE);
            item5.setText(0, Messages.MonitorSettingsTable_4);
            item5.setText(1, m_nodeAndBundleInfo.getFeatureSymbolicName().orElse("?")); //$NON-NLS-1$

            TableItem item6 = new TableItem(table, SWT.NONE);
            item6.setText(0, Messages.MonitorSettingsTable_6);
            item6.setText(1, m_nodeAndBundleInfo.getFeatureVersion().map(v -> v.toString()).orElse("?")); //$NON-NLS-1$

            TableItem item1 = new TableItem(table, SWT.NONE);
            item1.setText(0, Messages.MonitorSettingsTable_8);
            item1.setText(1, m_nodeAndBundleInfo.getBundleName().orElse("?")); //$NON-NLS-1$

            TableItem item2 = new TableItem(table, SWT.NONE);
            item2.setText(0, Messages.MonitorSettingsTable_10);
            item2.setText(1, m_nodeAndBundleInfo.getBundleSymbolicName().orElse("?")); //$NON-NLS-1$

            TableItem item3 = new TableItem(table, SWT.NONE);
            item3.setText(0, Messages.MonitorSettingsTable_12);
            item3.setText(1, m_nodeAndBundleInfo.getBundleVersion().map(v -> v.toString()).orElse("?")); //$NON-NLS-1$
        }
        ...
        while (!stack.isEmpty()) {
            ...
            if (confBase != null) {
                ...
                if ((!val.endsWith("_Internals")) || m_showAll) { //$NON-NLS-1$
            ...
            if ((!noexpertskip) || m_showAll) {
                ...
                if (second instanceof ConfigBase) {
                    ...
                    throw new IllegalStateException(
                        Messages.MonitorSettingsTable_15 + second.getClass() + Messages.MonitorSettingsTable_16);
                }
                TableItem item = new TableItem(table, SWT.NONE);
                char[] indent = new char[depth - 1];
                Arrays.fill(indent, '_');
                item.setText(0, new String(indent) + key);
                item.setText(1, value != null ? value : Messages.MonitorSettingsTable_17);
            }
        }
    ...
    public void updateControls(final Button loadButton, final Combo portCombo, final int count) {
        ...
        if (count == 0) {
            loadButton.setText(Messages.MonitorSettingsTable_18);
    ...
    public void updateInfoLabel(final Label info) {
        info.setText(Messages.MonitorSettingsTable_19);
    }
