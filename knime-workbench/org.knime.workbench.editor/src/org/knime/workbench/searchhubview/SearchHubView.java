...
public final class SearchHubView extends ViewPart {
    ...
    private static final String URL_PREFIX = "https://hub.knime.com/search?q="; //$NON-NLS-1$

    private static final String URL_POSTFIX = "&src=knimeapp"; //$NON-NLS-1$

    private static final String LINUX_LABEL_TEXT = Messages.SearchHubView_2;

    private static final String HINT_TEXT = Messages.SearchHubView_3;
            ...
            public void keyReleased(final KeyEvent ke) {
                if (ke.character == SWT.CR) {
                    ...
                    if (searchTerms.length() > 0) {
                        try {
                            Program.launch(URL_PREFIX + URLEncoder.encode(searchTerms, "UTF-8") + URL_POSTFIX); //$NON-NLS-1$
                            m_searchTextField.setText(""); //$NON-NLS-1$
                        } catch (final Exception e) {
                            LOGGER.error(Messages.SearchHubView_6 + e);
                        }
                    }
                }
            }
