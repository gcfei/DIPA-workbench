...
public class LinkMetaInfoAtom extends MetaInfoAtom {
    ...
    public static final String LEGACY_METADATA_LINK_BLOG_TYPE_KEYWORD = Messages.LinkMetaInfoAtom_0;
    /** The fixed acceptable link type for general Website links **/
    public static final String LEGACY_METADATA_LINK_URL_TYPE_KEYWORD = Messages.LinkMetaInfoAtom_1;
    /** The fixed acceptable link type for Video links **/
    public static final String LEGACY_METADATA_LINK_VIDEO_TYPE_KEYWORD = Messages.LinkMetaInfoAtom_2;
    ...
    private static final String URL_LEGACY_KEYWORD_TYPE_NAME = "Website"; //$NON-NLS-1$
    ...
    public LinkMetaInfoAtom(final String label, final String value, final String type, final String url,
        ...
        String urlToUse = url;
        if (url == null) {
            throw new MalformedURLException(Messages.LinkMetaInfoAtom_3);
        }
        ...
        if (((m_value == null) || (m_value.trim().length() == 0)) && (m_linkURL != null)) {
            index = m_linkURL.indexOf("//"); //$NON-NLS-1$
    ...
    private void updateDisplayText() {
        m_displayText = m_value + (((m_linkType != null) && !m_linkType.isEmpty()) ? " (" + m_linkType + ")" : ""); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    }
    ...
    public String getLegacyDescriptionRepresentation() {
        ...
        sb.append(DISPLAY_KEYWORD_BIDI_MAP.get(m_linkType)).append(": "); //$NON-NLS-1$

        if (titleMissing) {
            sb.append("A link"); //$NON-NLS-1$
        } else {
            sb.append(m_value);
        }
        sb.append(' ');
    ...
    protected void addAdditionalSaveTimeAttributes(final AttributesImpl attributes) {
        attributes.addAttribute(null, null, MetadataXML.URL_TYPE_ATTRIBUTE, "CDATA", m_linkType); //$NON-NLS-1$
        attributes.addAttribute(null, null, MetadataXML.URL_URL_ATTRIBUTE, "CDATA", m_linkURL); //$NON-NLS-1$
    }
