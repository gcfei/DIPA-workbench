...
public abstract class AbstractMetadataModelFacilitator implements MetaInfoAtom.MutationListener {
    ...
    public void parsingHasFinishedWithDefaultTitleName(final String defaultTitleName) {
        if (m_authorAtom == null) {
            m_authorAtom = new TextFieldMetaInfoAtom(MetadataItemType.AUTHOR, MetadataXML.AUTHOR_LABEL,
                System.getProperty("user.name"), false); //$NON-NLS-1$
            m_authorAtom.addChangeListener(this);
        }

        if (m_titleAtom == null) {
            m_titleAtom = new TextFieldMetaInfoAtom(MetadataItemType.TITLE, Messages.getString("AbstractMetadataModelFacilitator.1"), defaultTitleName, false); //$NON-NLS-1$
        ...
        if (m_licenseAtom == null) {
            m_licenseAtom =
                new ComboBoxMetaInfoAtom(Messages.getString("AbstractMetadataModelFacilitator.2"), LicenseType.DEFAULT_LICENSE_NAME, false); //$NON-NLS-1$
    ...
    public MetaInfoAtom addTag(final String tagText) {
        final TagMetaInfoAtom mia = new TagMetaInfoAtom(Messages.getString("AbstractMetadataModelFacilitator.3"), tagText, false); //$NON-NLS-1$
    ...
    public MetaInfoAtom addLink(final String url, final String title, final String type)
        ...
        final LinkMetaInfoAtom mia = new LinkMetaInfoAtom(Messages.getString("AbstractMetadataModelFacilitator.4"), title, type, url, false); //$NON-NLS-1$
        ...
        switch (deletedAtom.getType()) {
            case TAG:
                if (m_tagAtoms.remove(deletedAtom)) {
                    ...
                    m_logger.warn(Messages.getString("AbstractMetadataModelFacilitator.5") + deletedAtom.getValue() + Messages.getString("AbstractMetadataModelFacilitator.6")); //$NON-NLS-1$ //$NON-NLS-2$
                }
                break;
            case LINK:
                if (m_linkAtoms.remove(deletedAtom)) {
                    ...
                    m_logger.warn(Messages.getString("AbstractMetadataModelFacilitator.7") + deletedAtom.getValue() + Messages.getString("AbstractMetadataModelFacilitator.8")); //$NON-NLS-1$ //$NON-NLS-2$
                }
                break;
            default:
                m_logger.error(Messages.getString("AbstractMetadataModelFacilitator.9") + deletedAtom.getType() //$NON-NLS-1$
                    + Messages.getString("AbstractMetadataModelFacilitator.10")); //$NON-NLS-1$
                break;
        }
