...
public class NodeRecommendationManager {
    ...
    private static final String SOURCE_NODES_KEY = "<source_nodes>"; //$NON-NLS-1$

    private static final String NODE_NAME_SEP = "#"; //$NON-NLS-1$

    private static final String TRIPLE_PROVIDER_EXTENSION_POINT_ID = "org.knime.workbench.workflowcoach.nodetriples"; //$NON-NLS-1$
            ...
            if (WorkflowCoachPreferenceInitializer.P_COMMUNITY_NODE_TRIPLE_PROVIDER.equals(event.getKey())) {
                try {
                    ...
                    NodeLogger.getLogger(NodeRecommendationManager.class)
                        .error(Messages.NodeRecommendationManager_3 + ex.getMessage(), ex);
                }
            }
        };
        InstanceScope.INSTANCE.getNode("org.knime.workbench.workflowcoach").addPreferenceChangeListener(l); //$NON-NLS-1$
        DefaultScope.INSTANCE.getNode("org.knime.workbench.workflowcoach").addPreferenceChangeListener(l); //$NON-NLS-1$
    }
    ...
    private static boolean isSourceNode(final NodeInfo ni) {
        ...
        try {
            ...
            LOGGER.warn(Messages.NodeRecommendationManager_6 + ni.getFactory() + ": " + ex.getMessage(), ex); //$NON-NLS-2$
            return false;
        }
    }
        ...
        for (int idx = 0; idx < res.length; idx++) {
            if (nnc.length == 0) {
                ...
                throw new UnsupportedOperationException(
                    Messages.NodeRecommendationManager_8);
            }
    ...
    public List<NodeTripleProviderFactory> getNodeTripleProviderFactories() {
        ...
        assert (extPoint != null) : Messages.NodeRecommendationManager_9 + TRIPLE_PROVIDER_EXTENSION_POINT_ID;
            ...
            for (IConfigurationElement conf : ext.getConfigurationElements()) {
                try {
                    NodeTripleProviderFactory factory =
                        (NodeTripleProviderFactory)conf.createExecutableExtension("factory-class"); //$NON-NLS-1$
                    l.add(factory);
                } catch (CoreException e) {
                    LOGGER.warn(Messages.NodeRecommendationManager_11 + conf.getAttribute("factory-class")); //$NON-NLS-2$
                }
            }
        }
        return l;
