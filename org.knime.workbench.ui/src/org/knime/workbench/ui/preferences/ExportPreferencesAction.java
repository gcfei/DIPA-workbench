...
public class ExportPreferencesAction extends Action {
    ...
    private static final ImageDescriptor ICON =
            KNIMEUIPlugin.imageDescriptorFromPlugin(KNIMEUIPlugin.PLUGIN_ID,
                    "icons/prefs_export.png"); //$NON-NLS-1$

    /**
     * The id for this action.
     */
    public static final String ID = "KNIMEPreferencesExport"; //$NON-NLS-1$
    ...
    public ExportPreferencesAction(final IWorkbenchWindow window) {
        super(Messages.ExportPreferencesAction_2);
        if (window == null) {
            throw new IllegalArgumentException();
        }
        setToolTipText(Messages.ExportPreferencesAction_3);
        setId(ID); //$NON-NLS-1$
    }
    ...
    public void run() {
        ...
        if (!outFile.exists()) {
            ...
            if (!p.exists() && !outFile.getParentFile().mkdirs()) {
                MessageDialog.openError(workbenchWindow.getShell(),
                        Messages.ExportPreferencesAction_4,
                        Messages.ExportPreferencesAction_5);
                return;
            }
        }
        ...
        try {
            ...
            NodeLogger.getLogger(ExportPreferencesAction.class).info(
                    Messages.ExportPreferencesAction_6
                            + outFile.getAbsolutePath());
            /* Do not export the default values and the profile (contains only update site URLs). */
            prefService.exportPreferences(prefService.getRootNode(), out, new String[]{"bundle_defaults", "profile", //$NON-NLS-1$ //$NON-NLS-2$
                "instance/org.knime.product//us_info-"}); //$NON-NLS-1$
        } catch (Throwable t) {
            String msg = Messages.ExportPreferencesAction_10;
            if (t.getMessage() != null && !t.getMessage().isEmpty()) {
                msg = t.getMessage();
            }
            MessageDialog.openError(workbenchWindow.getShell(),
                    Messages.ExportPreferencesAction_11, msg);
            return;
