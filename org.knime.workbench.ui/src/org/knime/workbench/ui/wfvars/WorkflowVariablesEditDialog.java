...
public class WorkflowVariablesEditDialog extends Dialog {
    ...
    public void loadFrom(final FlowVariable var) {
        ...
        if (typeString == null) {
            throw new IllegalArgumentException(Messages.WorkflowVariablesEditDialog_0
                    + var.getName() + Messages.WorkflowVariablesEditDialog_1);
        }
    ...
    protected Control createDialogArea(final Composite parent) {
        parent.getShell().setText(Messages.WorkflowVariablesEditDialog_2);
        ...
        varNameLabel.setText(Messages.WorkflowVariablesEditDialog_3);
            ...
            public void modifyText(final ModifyEvent arg0) {
                ...
                for (Scope s : FlowVariable.Scope.values()) {
                    if (!Scope.Flow.equals(s) && text.startsWith(s.getPrefix())) {
                        showError(Messages.WorkflowVariablesEditDialog_4
                                + s.getPrefix() + "\"!"); //$NON-NLS-1$
                        m_varNameCtrl.setText(""); //$NON-NLS-1$
                    }
                }
            }
        ...
        typeLabel.setText(Messages.WorkflowVariablesEditDialog_7);
        ...
        defaultValueLabel.setText(Messages.WorkflowVariablesEditDialog_8);
    ...
    protected void okPressed() {
        ...
        if (varName.isEmpty()) {
            String msg = Messages.WorkflowVariablesEditDialog_9;
            showError(msg);
            throw new OperationCanceledException(msg);
        }
        String value = m_varDefaultValueCtrl.getText().trim();
        if (value.isEmpty()) {
            String msg = Messages.WorkflowVariablesEditDialog_10;
            showError(msg);
            throw new OperationCanceledException(msg);
        }
        int selectionIdx = m_typeSelectionCtrl.getSelectionIndex();
        if (selectionIdx < 0) {
            String msg = Messages.WorkflowVariablesEditDialog_11
                + varName;
            showError(msg);
        ...
        if (FlowVariable.Type.DOUBLE.equals(m_type)) {
            try {
               m_variable = new FlowVariable(varName,
                ...
                m_variable = null;
                String msg = Messages.WorkflowVariablesEditDialog_12 + value
                    + Messages.WorkflowVariablesEditDialog_13 + varName + "!"; //$NON-NLS-2$
                showError(msg);
                throw new OperationCanceledException(msg);
            }
           ...
           m_variable = new FlowVariable(varName, value);
            ...
            try {
               m_variable = new FlowVariable(varName, Integer.parseInt(
                ...
                m_variable = null;
                String msg = Messages.WorkflowVariablesEditDialog_15 + value
                    + Messages.WorkflowVariablesEditDialog_16 + varName + "!"; //$NON-NLS-2$
                showError(msg);
                throw new OperationCanceledException(msg);
            }
        }
    ...
    private void showError(final String message) {
        MessageDialog.openError(getParentShell(), Messages.WorkflowVariablesEditDialog_18, message);
    }
