...
class MetadataModelFacilitator extends AbstractMetadataModelFacilitator {
    ...
    private static final String LEGACY_METADATA_TAG_KEYWORD = "TAG"; //$NON-NLS-1$
    private static final String LEGACY_METADATA_TAGS_KEYWORD = "TAGS"; //$NON-NLS-1$

    private static final String LEGACY_METADATA_LICENSE_KEYWORD = "LICENSE"; //$NON-NLS-1$
            ...
            switch (typeToUse) {
                ...
                case LINK:
                    try {
                        ...
                        m_logger.error(Messages.MetadataModelFacilitator_0 + url + "]", e); //$NON-NLS-2$
                    }
                    break;
                case AUTHOR:
    ...
    void writeMetadata(final File metadataFile) throws IOException {
        try (final OutputStream os = new FileOutputStream(metadataFile)) {
            ...
            throw new IOException(Messages.MetadataModelFacilitator_1, e);
        }
    }
    ...
    private void writeLegacyStyleMetadataToStream(final OutputStream os) throws IOException {
        try {
            ...
            attributes.addAttribute(null, null, MetadataXML.FORM, "CDATA", MetadataXML.MULTILINE); //$NON-NLS-1$
            attributes.addAttribute(null, null, MetadataXML.READ_ONLY, "CDATA", Boolean.toString(false)); //$NON-NLS-1$
            attributes.addAttribute(null, null, MetadataXML.NAME, "CDATA", MetadataXML.DESCRIPTION_LABEL); //$NON-NLS-1$
...
//            attributes.addAttribute(null, null, MetadataXML.TYPE, "CDATA", m_descriptionAtom.getType().getType());
            ...
            throw new IOException(Messages.MetadataModelFacilitator_2, e);
        }
    }
    ...
    private String potentiallyParseOldStyleDescription(final String description) {
        ...
        if (isOldStyle) {
            ...
            m_titleAtom = new TextFieldMetaInfoAtom(MetadataItemType.TITLE, Messages.MetadataModelFacilitator_3, titleLine, false);
                ...
                if ((index != -1) && (index < (line.length() - 2))) {
                    ...
                    if (LEGACY_KEYWORDS.contains(initialText)) {
                        if (initialText.equals(LEGACY_METADATA_TAG_KEYWORD)
                            ...
                            final String[] tags = tagsConcatenated.split(","); //$NON-NLS-1$

                            for (final String tag : tags) {
                                addTag(tag.trim());
                            }
                        } else if (initialText.equals(LEGACY_METADATA_LICENSE_KEYWORD)) {
                            m_licenseAtom = new ComboBoxMetaInfoAtom(Messages.MetadataModelFacilitator_4,
                            ...
                            int urlStart = lowercaseLine.indexOf("http:"); //$NON-NLS-1$
                            if (urlStart == -1) {
                                urlStart = lowercaseLine.indexOf("https:"); //$NON-NLS-1$
                            }

                            if (urlStart == -1) {
                                m_logger.warn(Messages.MetadataModelFacilitator_5 + line
                                                    + "]"); //$NON-NLS-1$
                            } else {
                                ...
                                try {
                                    ...
                                    m_logger.error(Messages.MetadataModelFacilitator_6 + url + "]", e); //$NON-NLS-2$
                                }
                            }
                        }
    ...
    private String createOldStyleDescriptionBlock() {
        ...
        if (m_tagAtoms.size() > 0) {
            sb.append(LEGACY_METADATA_TAGS_KEYWORD).append(": "); //$NON-NLS-1$
