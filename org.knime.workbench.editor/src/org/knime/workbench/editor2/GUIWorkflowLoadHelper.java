...
class GUIWorkflowLoadHelper extends WorkflowLoadHelper {
    ...
    private static final String LOAD_FUTURE_FLOW_TEXT =
        Messages.GUIWorkflowLoadHelper_0
            + Messages.GUIWorkflowLoadHelper_1
            + Messages.GUIWorkflowLoadHelper_2;
    ...
    public UnknownKNIMEVersionLoadPolicy getUnknownKNIMEVersionLoadPolicy(final LoadVersion workflowKNIMEVersion,
        ...
        assert isFuture || isNightlyBuild : Messages.GUIWorkflowLoadHelper_3;
    ...
    private UnknownKNIMEVersionLoadPolicy shouldLoadFutureVersionWorkflow(final LoadVersion workflowKNIMEVersion,
        final Version createdByKNIMEVersion, final boolean isNightlyBuild) {
        StringBuilder e = new StringBuilder(Messages.GUIWorkflowLoadHelper_4);
        e.append(CURRENT_VERSION);
        e.append(Messages.GUIWorkflowLoadHelper_5);
        e.append(Messages.GUIWorkflowLoadHelper_6);
        e.append(createdByKNIMEVersion);
        if (isNightlyBuild) {
            e.append("-nightly"); //$NON-NLS-1$
        }
        e.append(").\n\n"); //$NON-NLS-1$

        e.append(Messages.GUIWorkflowLoadHelper_9);
        e.append(LOAD_FUTURE_FLOW_TEXT);

        String cancel = "&Cancel"; //$NON-NLS-1$
        String loadAnyway = "&Load Anyway"; //$NON-NLS-1$
        ...
        m_display.syncExec(new Runnable() {
            ...
            public void run() {
                MessageDialog dialog = new MessageDialog(SWTUtilities.getActiveShell(m_display),
                    Messages.GUIWorkflowLoadHelper_12,
    ...
    private UnknownKNIMEVersionLoadPolicy shouldLoadNightlyBuildWorkflow(final LoadVersion workflowKNIMEVersion,
        ...
        StringBuilder e = new StringBuilder(Messages.GUIWorkflowLoadHelper_13);
        if (createdByKNIMEVersion != null) {
            e.append(createdByKNIMEVersion);
        } else {
            e.append("<unknown>"); //$NON-NLS-1$
        }
        e.append(Messages.GUIWorkflowLoadHelper_15);
        e.append(LOAD_FUTURE_FLOW_TEXT);
        ...
        m_display.syncExec(new Runnable() {
            ...
            public void run() {
                MessageDialogWithToggle dialog = new MessageDialogWithToggle(SWTUtilities.getActiveShell(m_display),
                    Messages.GUIWorkflowLoadHelper_17,
                    null, e.toString(), MessageDialog.WARNING, labels, 0,
                    Messages.GUIWorkflowLoadHelper_18, false);
                switch (dialog.open()) {
