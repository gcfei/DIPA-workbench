...
public final class WizardNodeView<T extends ViewableModel & WizardNode<REP, VAL>,
    ...
    public final void callOpenView(final String title, final Rectangle knimeWindowBounds) {
        m_title = (title == null ? Messages.WizardNodeView_0 : title);
        ...
        m_browser.setText(getViewCreator().createMessageHTML(Messages.WizardNodeView_1), true);
        ...
        resetButton.setText(Messages.WizardNodeView_2);
        ...
        applyButton.setText(Messages.WizardNodeView_3);
        applyButton.setToolTipText(Messages.WizardNodeView_4);
        DropdownSelectionListener applyListener = new DropdownSelectionListener(applyButton);
        String aTTooltip = Messages.WizardNodeView_5;
        applyListener.add(Messages.WizardNodeView_6, aTTooltip, new SelectionAdapter() {
            @Override
            public void widgetSelected(final SelectionEvent e) {
                applyTriggered(false);
            }
        });
        String nDTooltip = Messages.WizardNodeView_7;
        applyListener.add(Messages.WizardNodeView_8, nDTooltip, new SelectionAdapter() {
            @Override
            public void widgetSelected(final SelectionEvent e) {
                applyTriggered(true);
            }
        ...
        closeButton.setText(Messages.WizardNodeView_9);
        closeButton.setToolTipText(Messages.WizardNodeView_10);
        DropdownSelectionListener closeListener = new DropdownSelectionListener(closeButton);
        String cDTooltip = Messages.WizardNodeView_11;
        closeListener.add(Messages.WizardNodeView_12, cDTooltip, new SelectionAdapter() {
            @Override
            public void widgetSelected(final SelectionEvent e) {
                m_shell.dispose();
            }
        });
        String cATooltip = Messages.WizardNodeView_13;
        closeListener.add(Messages.WizardNodeView_14, cATooltip, new SelectionAdapter() {
            @Override
            public void widgetSelected(final SelectionEvent e) {
        ...
        String cTTooltip = Messages.WizardNodeView_15;
        closeListener.add(Messages.WizardNodeView_16, cTTooltip, new SelectionAdapter() {
            @Override
            public void widgetSelected(final SelectionEvent e) {
        ...
        display.asyncExec(new Runnable() {
            ...
            public void run() {
                ...
                setBrowserURL();
                m_viewRequestCallback = new ViewRequestFunction(m_browser, "knimeViewRequest"); //$NON-NLS-1$
                m_updateRequestStatusCallback = new UpdateRequestStatusFunction(m_browser, "knimeUpdateRequestStatus"); //$NON-NLS-1$
                m_cancelRequestCallback = new CancelRequestFunction(m_browser, "knimeCancelRequest"); //$NON-NLS-1$
                m_isPushSupportedCallback = new PushSupportedFunction(m_browser, "knimePushSupported"); //$NON-NLS-1$
            }
        });
    ...
    private void setBrowserURL() {
        try {
            ...
            if (src != null && src.exists()) {
                ...
                m_browser.setText(getViewCreator().createMessageHTML(Messages.WizardNodeView_21));
                m_viewSet = false;
            }
        } catch (Exception e) {
    ...
    protected boolean showApplyOptionsDialog(final boolean showDiscardOption, final String title, final String message) {
        ...
        RadioItem applyOption = new RadioItem(APPLY_LABEL, null,
            String.format(APPLY_DESCRIPTION_FORMAT, showDiscardOption ? Messages.WizardNodeView_22 : "")); //$NON-NLS-2$
        RadioItem newDefaultOption = new RadioItem(APPLY_DEFAULT_LABEL, null,
            String.format(APPLY_DEFAULT_DESCRIPTION_FORMAT, showDiscardOption ? Messages.WizardNodeView_24 : "")); //$NON-NLS-2$
    ...
    protected boolean validateCurrentValueInView() {
        ...
        if (validateMethod != null && !validateMethod.isEmpty()) {
            String evalCode = creator
                .wrapInTryCatch("return JSON.stringify(" + creator.getNamespacePrefix() + validateMethod + "());"); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    protected String retrieveCurrentValueFromView() {
        ...
        if (ns != null && !ns.isEmpty() && pullMethod != null && !pullMethod.isEmpty()) {
            String evalCode = creator.wrapInTryCatch("if (typeof " + ns.substring(0, ns.length() - 1) //$NON-NLS-1$
                + " != 'undefined') { return JSON.stringify(" + ns + pullMethod + "());}"); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    protected void showValidationErrorInView(final String error) {
        ...
        String showErrorCall = creator.wrapInTryCatch(creator.getNamespacePrefix() + showErrorMethod + "('" + escapedError + "');"); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public void respondToViewRequest(final String response) {
        ...
        display.asyncExec(new Runnable() {
            ...
            @Override
            public void run() {
                LOGGER.debug("Sending response: " + response); //$NON-NLS-1$
                String call = "KnimeInteractivity.respondToViewRequest(JSON.parse('" + response + "'));"; //$NON-NLS-1$ //$NON-NLS-2$
                WizardViewCreator<REP, VAL> creator = getViewCreator();
    ...
    public void pushRequestUpdate(final String monitor) {
        ...
        display.asyncExec(new Runnable() {
            ...
            @Override
            public void run() {
                String call = "KnimeInteractivity.updateResponseMonitor(JSON.parse('" + monitor + "'));"; //$NON-NLS-1$ //$NON-NLS-2$
                WizardViewCreator<REP, VAL> creator = getViewCreator();
                call = creator.wrapInTryCatch(call);
                if (m_browser != null && !m_browser.isDisposed()) {
                    m_browser.execute(call);
