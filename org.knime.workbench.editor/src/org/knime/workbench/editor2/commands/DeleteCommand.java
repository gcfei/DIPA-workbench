...
public class DeleteCommand extends AbstractKNIMECommand {
    ...
    public void execute() {
        ...
        if (m_nodeIDs.length > 0 || m_annotationIDs.length > 0) {
            ...
            try {
                m_undoCopy = wfmAsyncSwitchRethrow(wfm -> wfm.cut(content.build()), wfm -> wfm.cutAsync(content.build()),
                    hostWFM, Messages.DeleteCommand_0);
            } catch (OperationNotAllowedException e) {
                openDialog(Messages.DeleteCommand_1, e.getMessage());
                return;
            }
        }
        ...
        try {
            wfmAsyncSwitchRethrow(wfm -> {
                ...
                Messages.DeleteCommand_2);
        } catch (OperationNotAllowedException e) {
            openDialog(Messages.DeleteCommand_3, e.getMessage());
        }
    }
    ...
    public void undo() {
        ...
        }, hostWFM, Messages.DeleteCommand_4);
    }
