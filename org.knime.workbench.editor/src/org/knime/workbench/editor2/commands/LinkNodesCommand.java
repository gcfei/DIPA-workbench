        ...
        for (final LinkNodesAction.PlannedConnection planAction : m_connectionPlans) {
            ...
            if (planAction.shouldDetachDestinationFirst()) {
                ...
                try {
                    ...
                    LOGGER.error(Messages.LinkNodesCommand_0 + destinationNodeID + ":" //$NON-NLS-2$
                        + planAction.getDestinationInportIndex() + Messages.LinkNodesCommand_2 + sourceNodeID
                        + ":" + planAction.getSourceOutportIndex() + Messages.LinkNodesCommand_4 + destinationNodeID + ":" //$NON-NLS-1$ //$NON-NLS-3$
                        + planAction.getDestinationInportIndex() + Messages.LinkNodesCommand_6 + e.getMessage(), e);
                    continue;
                }
            }
            ...
            try {
                ...
                LOGGER.error(Messages.LinkNodesCommand_7 + sourceNodeID + ":" + planAction.getSourceOutportIndex() + Messages.LinkNodesCommand_9 //$NON-NLS-2$
                    + destinationNodeID + ":" + planAction.getDestinationInportIndex() + Messages.LinkNodesCommand_11 + e.getMessage(), //$NON-NLS-1$
                    e);
            }
        }
    }
    ...
    public void undo() {
        ...
        for (final LinkNodesAction.PlannedConnection planAction : m_connectionPlans) {
            ...
            try {
                ...
                LOGGER.error(Messages.LinkNodesCommand_12 + cc.getSource() + ":" + cc.getSourcePort() //$NON-NLS-2$
                    + Messages.LinkNodesCommand_14 + cc.getDest() + ":" + cc.getDestPort() + Messages.LinkNodesCommand_16 + e.getMessage(), e); //$NON-NLS-2$
            }
        }
        ...
        for (final ConnectionContainer cc : m_removedConnections) {
            try {
                ...
                LOGGER.error(Messages.LinkNodesCommand_17 + cc.getSource() + ":" + cc.getSourcePort() //$NON-NLS-2$
                    + Messages.LinkNodesCommand_19 + cc.getDest() + ":" + cc.getDestPort() + Messages.LinkNodesCommand_21 + e.getMessage(), e); //$NON-NLS-2$
            }
        }
    }
}
