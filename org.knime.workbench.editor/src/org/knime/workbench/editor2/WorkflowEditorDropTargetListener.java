...
public abstract class WorkflowEditorDropTargetListener<T extends CreationFactory> extends
    ...
    protected ConfigurableNodeFactory<NodeModel> getNodeFactory(final URL url) {
        ...
        if (clazz == null) {
            LOGGER.warn(Messages.WorkflowEditorDropTargetListener_0 + " \"" + path + "\""); //$NON-NLS-2$ //$NON-NLS-3$
            return null;
        }
        try {
            return clazz.newInstance();
        } catch (InstantiationException e) {
            LOGGER.error(Messages.WorkflowEditorDropTargetListener_3 + clazz.getName() + ".", e); //$NON-NLS-2$
