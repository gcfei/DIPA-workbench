...
public final class OpenInteractiveWebViewAction extends Action {
    ...
    private static final String CHROMIUM_BROWSER = "org.knime.ext.seleniumdrivers.multios.ChromiumWizardNodeView"; //$NON-NLS-1$
    private static final String CHROME_BROWSER = "org.knime.ext.seleniumdrivers.multios.ChromeWizardNodeView"; //$NON-NLS-1$
    ...
    public String getToolTipText() {
        return Messages.OpenInteractiveWebViewAction_3 + m_webViewForNode.getViewName();
    }
    ...
    public String getText() {
        ...
        if (!m_singleTitle) {
            return Messages.OpenInteractiveWebViewAction_4 + text;
        } else {
            return text;
        }
    }
    ...
    @Override
    public void run() {
        LOGGER.debug(Messages.OpenInteractiveWebViewAction_5 + m_nodeContainer.getName());
        if (wraps(m_nodeContainer, NodeContainer.class)) {
            ...
            try {
                PlatformUI.getWorkbench().getProgressService().busyCursorWhile((monitor) -> {
                    monitor.beginTask(Messages.OpenInteractiveWebViewAction_6, 100);
    ...
    private static void showWarningDialog(final NodeContainerUI nc, final Throwable t) {
        ...
        mb.setText(Messages.OpenInteractiveWebViewAction_7);
        mb.setMessage(Messages.OpenInteractiveWebViewAction_8 + t.getMessage());
        mb.open();
        StringBuilder sb = new StringBuilder(Messages.OpenInteractiveWebViewAction_9);
        sb.append(nc.getNameWithID());
        sb.append(Messages.OpenInteractiveWebViewAction_10);
        sb.append(t.getClass().getSimpleName());
        sb.append(Messages.OpenInteractiveWebViewAction_11);
        if (Wrapper.wraps(nc, NodeContainer.class)) {
            sb.append(Messages.OpenInteractiveWebViewAction_12);
    ...
    static AbstractWizardNodeView getConfiguredWizardNodeView(final ViewableModel model) {
        ...
        IConfigurationElement[] configurationElements =
                registry.getConfigurationElementsFor("org.knime.core.WizardNodeView"); //$NON-NLS-1$
        ...
        if (StringUtils.isNotEmpty(classString)) {
            ...
            if (viewClass == null) {
                LOGGER.error(Messages.OpenInteractiveWebViewAction_14 + classString
                    + Messages.OpenInteractiveWebViewAction_15);
            }
        }
        if (viewClass == null) {
        ...
        if (viewClass != null) {
            try {
                ...
                if (!isEnabled) {
                    LOGGER.error(Messages.OpenInteractiveWebViewAction_18 + classString
                        + Messages.OpenInteractiveWebViewAction_19);
                    viewClass = null;
                }
            } catch (Exception e) { /*do nothing */ }
        }
        if (viewClass != null) {
            try {
                ...
                LOGGER.error(Messages.OpenInteractiveWebViewAction_20);
            }
        }
    ...
    private static Class<?> getViewClassByReflection(final String className, final IConfigurationElement[] confElements) {
        ...
        try {
            for (IConfigurationElement element : confElements) {
                if (className.equals(element.getAttribute("viewClass"))) { //$NON-NLS-1$
                    viewClass = Platform.getBundle(element.getDeclaringExtension().getContributor().getName())
                        .loadClass(element.getAttribute("viewClass")); //$NON-NLS-1$
                }
            }
        } catch (Exception e) { /* do nothing */}
        if (viewClass != null) {
            try {
                Method isEnabledM = viewClass.getMethod("isEnabled"); //$NON-NLS-1$
                boolean isEnabled = (boolean)isEnabledM.invoke(null);
    ...
    @Override
    public String getId() {
        return "knime.open.interactive.web.view.action"; //$NON-NLS-1$
    }
}
