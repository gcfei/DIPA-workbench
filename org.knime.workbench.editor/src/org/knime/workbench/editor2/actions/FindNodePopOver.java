...
public class FindNodePopOver extends PopupDialog {
    ...
    private static final Pattern NODE_ID_PATTERN = Pattern.compile("^[\\d]{1,2}:?"); //$NON-NLS-1$
    private static final Pattern EASTER_EGG_NODE_ID_PATTERN = Pattern.compile("^[\\d]{1,2}:[\\d]{1,8}"); //$NON-NLS-1$
    ...
    public FindNodePopOver(final WorkflowEditor workflowEditor) {
        ...
        if (!workflowManager.isPresent()) {
            throw new IllegalStateException(Messages.FindNodePopOver_2);
        }
    ...
    private void searchRobustlyForNode(final String id) {
        ...
        try {
            ...
            LOGGER.debug(Messages.FindNodePopOver_3 + id, e);
            nc = null;
        }

        if (nc == null) {
            MessageDialog.openWarning(SWTUtilities.getKNIMEWorkbenchShell(), Messages.FindNodePopOver_4,
                Messages.FindNodePopOver_5 + id);
        } else {
            ...
            if (parent.equals(projectWM)) {
                ...
                if (we != null) {
                    ...
                    MessageDialog.openInformation(SWTUtilities.getKNIMEWorkbenchShell(), Messages.FindNodePopOver_6,
                        Messages.FindNodePopOver_7);
                }
            } else {
                ...
                try {
                    ...
                    LOGGER.error(Messages.FindNodePopOver_8, e);
                }
            }
    ...
    private static class NodeContainerDisplayWrapper {
        ...
        private NodeContainerDisplayWrapper(final NodeContainerUI ncUI, final Display display) {
            ...
            if (!m_renderLowerText) {
                m_displayText = name + " (" + nid + ")"; //$NON-NLS-1$ //$NON-NLS-2$
                ...
                if (fullAnnotationText.length() > MAX_CHARACTER_LENGTH_FOR_NODE_ANNOTATION_IN_LABEL) {
                    displayAnnotationText =
                        fullAnnotationText.substring(0, MAX_CHARACTER_LENGTH_FOR_NODE_ANNOTATION_IN_LABEL) + "..."; //$NON-NLS-1$
                } else {
                    displayAnnotationText = fullAnnotationText;
                }
                m_displayText = name + " " + displayAnnotationText + " (" + nid + ")"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                final String searchText = name + " " + fullAnnotationText + " (" + nid + ")"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
