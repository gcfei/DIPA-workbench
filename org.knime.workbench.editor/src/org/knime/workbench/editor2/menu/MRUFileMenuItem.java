...
//            }
    ...
    public void fill(final Menu menu, final int index) {
        ...
        while (iterator.hasPrevious()) {
            ...
            final String[] parts = uri.split("/"); //$NON-NLS-1$

            try {
                final URL u = new URL(uri);
                final MenuItem mi = new MenuItem(menu, SWT.PUSH);
                mi.setText(URLDecoder.decode(parts[parts.length - 2], "UTF-8")); //$NON-NLS-1$
                mi.setToolTipText(URLDecoder.decode(uri, "UTF-8").replaceAll("/workflow\\.knime$", "")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                mi.setEnabled(false);
                    ...
                    public void widgetSelected(final SelectionEvent se) {
                        try {
                            final URI workflowURI = new URI(u.getProtocol(), u.getHost(),
                                URLDecoder.decode(u.getPath(), "UTF-8"), u.getQuery()); //$NON-NLS-1$
                            IDE.openEditor(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage(),
                                workflowURI, WorkflowEditor.ID, true);
                        } catch (final Exception exception) {
                            LOGGER.error(Messages.MRUFileMenuItem_6
                ...
                LOGGER.error(Messages.MRUFileMenuItem_7, e);
            }
        }
    }
    ...
    private synchronized void assureMapInitialPopulation() {
        if (m_recentURIMap == null) {
            ...
            try {
                ...
                Document doc;
                try {
                    final Path workbenchPath = parentPath.resolve("org.eclipse.e4.workbench").resolve("workbench.xmi"); //$NON-NLS-1$ //$NON-NLS-2$
                    ...
                    LOGGER.info(Messages.MRUFileMenuItem_10, exception);
                    return;
                }

                final XPath xpath = XPathFactory.newInstance().newXPath();
                final String mruMemento = (String)xpath.evaluate("persistedState[@key = 'memento']/@value", //$NON-NLS-1$
                ...
                final NodeList workflowList =
                    (NodeList)xpath.evaluate("//mruList/file[@id = '" + WorkflowEditor.ID + "']/persistable", //$NON-NLS-1$ //$NON-NLS-2$
                ...
                if (workflowList.getLength() > 0) {
                    ...
                    for (int i = (count - 1); i >= 0; i--) {
                        ...
                        final String uri = e.getAttribute("uri"); // knime://MP/.../WorkflowName/workflow.knime //$NON-NLS-1$
                        final String[] parts = uri.split("/"); //$NON-NLS-1$
                        if (parts.length > 2) {
                            m_recentURIMap.put(uri, new Object());
                        }
                ...
                LOGGER.error(Messages.MRUFileMenuItem_16, e);
            }
        }
    }
    ...
    private static class AssetExistenceResolver implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                LOGGER.debug(Messages.MRUFileMenuItem_17 + m_assetURL.toExternalForm() + Messages.MRUFileMenuItem_18,
                    ioe);
