...
public class MonitorDataTable implements NodeMonitorTable {
    ...
    public void loadTableData(final NodeContainerUI ncUI, final NodeContainer nc, final int count)
        ...
        if (count == 0) {
            ...
            if (ncUI.getNrOutPorts() <= index) {
                ...
                throw new LoadingFailedException(Messages.MonitorDataTable_0);
            }
            if (!ncUI.getNodeContainerState().isExecuted()) {
                throw new LoadingFailedException(Messages.MonitorDataTable_1);
            }
            ...
            if (nop instanceof AsyncNodeOutPortUI) {
                try {
                    ...
                    throw new LoadingFailedException(Messages.MonitorDataTable_2 + e.getMessage());
                }
            } else {
            ...
            if (!((po instanceof BufferedDataTable) || (po instanceof KnowsRowCountTable))) {
                ...
                throw new LoadingFailedException(Messages.MonitorDataTable_3);
            }
            // retrieve table
    ...
    public void setupTable(final Table table) {
        ...
        if (table.getColumnCount() >= MAX_NUM_COLUMN - 1) {
            column = new TableColumn(table, SWT.NONE);
            column.setText(Messages.MonitorDataTable_5);
        }
    ...
    public void updateControls(final Button loadButton, final Combo portCombo, final int count) {
        if (count == 0) {
            loadButton.setText(Messages.MonitorDataTable_6);
        } else {
            loadButton.setText(Messages.MonitorDataTable_7);
        }
    ...
    public void updateInfoLabel(final Label info) {
        info.setText(Messages.MonitorDataTable_8);
    }
        ...
        public void handleEvent(final Event event) {
            ...
            if (row != null) {
                ...
                for (int i = 0; i < colCount; i++) {
                    ...
                    String s = c.toString().replaceAll("\\p{Cntrl}", "_"); //$NON-NLS-1$ //$NON-NLS-2$
                    item.setText(i + 1, s);
                }
                if (m_table.getColumnCount() == MAX_NUM_COLUMN) {
                    item.setText(MAX_NUM_COLUMN - 1, "..."); //$NON-NLS-1$
                }
            } else {
                item.setText(Messages.MonitorDataTable_12 + m_currentIndex + Messages.MonitorDataTable_13);
            }
        }
    }
}
